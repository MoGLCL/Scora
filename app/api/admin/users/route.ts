...
export async function GET() {
  ...
  const rows = await query<{
    ...
    created_at: Date; skill_points: number | null; trust_score: number | null; reports_count: number; approval_status:string|null; assessment_public_id:string|null;
  ...
  return NextResponse.json(rows.map((row) => {
    ...
    return { id: String(row.id), name: row.full_name, email: row.email, role: row.role, isAdmin: Boolean(row.is_admin),
      ...
      reportsCount: Number(row.reports_count), joinDate: joined, joinedDate: joined, approvalStatus:row.approval_status, assessmentPublicId:row.assessment_public_id };
  }));
}
