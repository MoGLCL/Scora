...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (auth.role !== "developer") redirect("/dashboard");

  const { id } = await params;
  if (!id || id === "pending" || !id.startsWith("assess_")) {
  ...
  const session = await queryOne<{
    ...
    "SELECT das.id, das.developer_id, das.status, GREATEST(0, TIMESTAMPDIFF(SECOND, CURRENT_TIMESTAMP, das.expires_at)) remaining_seconds FROM developer_assessment_sessions das JOIN developers d ON d.id = das.developer_id WHERE das.public_id = ? AND d.user_id = ?",
    [id, auth.userId]
  );

  if (!session) notFound();
  if (session.status !== "in_progress") redirect("/developer-assessment/pending");
  ...
  return (
    <main dir="rtl" className="mx-auto max-w-5xl px-6 py-12 font-body">
      <h1 className="text-3xl font-extrabold text-[#05291A]">اختبار اعتماد المطور</h1>
      <p className="mb-8 mt-2 text-sm text-gray-600">
        تم إنشاء الاختبار بواسطة AI حسب مهاراتك. يتم حفظ كل إجابة تلقائيًا، ولا يمكن تغيير الاختبار بعد بدء الجلسة.
      </p>
