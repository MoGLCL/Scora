...
export async function sendMessage(input:{receiverId:number|string;body:string}){
  ...
  const receiver=await queryOne<{id:number;role:"developer"|"client"}>("SELECT id,role FROM users WHERE id=? AND status='active' AND onboarding_completed_at IS NOT NULL",[parsed.data.receiverId]);
