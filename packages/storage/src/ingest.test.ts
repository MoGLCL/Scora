...
import {
  ...
  silentLogger,
} from '@scora/trust-core/node';
import { DEV, SESSION, TENANT } from './conformance.ts';
import { createIngestion, type Ingestion } from './ingest.ts';
...
function submission(sequence: number, overrides: Record<string, unknown> = {}): unknown {
  return {
...
function externalSubmission(sequence: number): unknown {
...
async function typesIn(store: EventStore): Promise<string[]> {
...
describe('ingestion — the happy path', () => {
  ...
  it('continues the chain across separate calls', async () => {
    ...
    assert.equal(
      verifyChain(stored, nodeCrypto.sha256).intact,
      true,
      'a batch boundary is not a chain boundary',
    );
  });
  ...
  it('records a null offset rather than pretending the timing is exact', async () => {
    ...
    assert.equal(
      stored!.occurredAtNormalized,
      stored!.occurredAt,
      'an unknown offset lowers confidence; it does not invent a correction',
    );
  });
});
...
describe('ingestion — malformed telemetry becomes evidence, not silence', () => {
  ...
  it('records the rejection in the log so confidence can fall', async () => {
    ...
    assert.ok(
      (await typesIn(store)).includes(TrustEventType.EVENT_REJECTED),
      'a session with partly broken telemetry must not read as clean',
    );
  });
  ...
  it('reports a fully-rejected batch without inventing a session', async () => {
    ...
    assert.deepEqual(
      await store.readSession(TENANT, SESSION),
      [],
      'nothing identified a session, so there is no log to write into',
    );
  });
  ...
  it('refuses a batch spanning two sessions', async () => {
    ...
    await assert.rejects(
      () => ingestion.ingest([submission(1), submission(2, { sessionId: 'sess_elsewhere' })]),
...
describe('ingestion — lost telemetry lowers confidence, it does not raise risk', () => {
  ...
  it('does not report a gap for a complete batch', async () => {
    ...
    assert.equal(
      (await typesIn(store)).includes(TrustEventType.EVENT_SEQUENCE_GAP),
      false,
      'a clean session must produce no system events at all',
    );
  });
...
describe('ingestion — consent gates collection, not reading', () => {
  const externalGranted = {
    async grantedScopes() {
      return ['assessment', 'external_monitoring'] as const;
    },
  };
  ...
  it('never stores what consent did not cover', async () => {
    ...
    assert.equal(
      (await typesIn(store)).includes(TrustEventType.EXTERNAL_RESOURCE_ACCESSED),
      false,
      'filtering at read time would leave unlawful data sitting in storage',
    );
  });
...
describe('ingestion — the server must not consume the client sequence space', () => {
  it('accepts the next client batch after a rejection was recorded', async () => {
    ...
    const stored = await store.readSession(TENANT, SESSION);
    assert.deepEqual(
      stored.filter((event) => event.source === EventSource.SANDBOX).map((event) => event.sequence),
      [1, 2, 3],
    );
  });
