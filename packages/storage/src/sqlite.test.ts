...
import { DEV, SESSION, TENANT, makeChain, runStoreConformance } from './conformance.ts';
import { sqliteEventStore } from './sqlite.ts';
...
describe('sqliteEventStore — persistence specifics', () => {
  ...
  it('rolls back a failed batch entirely', async () => {
    ...
    const poisoned = { ...events[1]!, payload: { bad: BigInt(1) } } as unknown as (typeof events)[number];
