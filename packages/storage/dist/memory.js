...
export function inMemoryEventStore() {
    ...
    const sessionKey = (tenantId, sessionId) => `${tenantId}::${sessionId}`;
    const idKey = (tenantId) => tenantId;
    function idsFor(tenantId) {
    ...
    return {
        ...
        async read(query) {
            ...
            const types = query.types === undefined ? null : new Set(query.types);
            ...
            const matching = record.events.filter((event) => event.chainPosition >= from &&
            ...
            const nextCursor = page.length < matching.length ? String(page.at(-1).chainPosition) : null;
        ...
        async listSessions(tenantId, developerId, options) {
            ...
            for (const record of sessions.values()) {
                if (record.tenantId !== tenantId || record.developerId !== developerId)
                    continue;
        ...
        async eraseSubject(tenantId, developerId) {
            ...
            for (const [key, record] of sessions) {
                ...
                for (const event of record.events)
                    idsFor(tenantId).delete(event.eventId);
                sessions.delete(key);
            }
...
function hasContinuousChain(events) {
    for (let index = 1; index < events.length; index += 1) {
        if (events[index].chainPosition !== events[index - 1].chainPosition + 1)
            return false;
    }
    return true;
}
