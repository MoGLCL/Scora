...
import { DeveloperId, EventId, SessionId, TenantId, unsafeEpochMs, } from '@scora/trust-core';
/**
...
export function sqliteEventStore(location = ':memory:') {
  ...
  `);
    return {
        async append(events) {
            ...
            try {
                for (const event of events) {
                    const result = insert.run(event.tenantId, event.sessionId, event.eventId, event.developerId, event.assessmentId ?? null, event.taskId ?? null, event.type, event.layer, event.sequence, event.chainPosition, event.occurredAt, event.occurredAtNormalized, event.receivedAt, event.clockOffsetMs, event.source, event.schemaVersion, JSON.stringify(event.payload), JSON.stringify(event.redactedFields), event.integrity.hash, event.integrity.previousHash);
        ...
        async head(tenantId, sessionId) {
            const row = selectHead.get(tenantId, sessionId, tenantId, sessionId, tenantId, sessionId);
            if (row === undefined)
                return null;
            return {
                sessionId,
        ...
        async existing(tenantId, ids) {
            ...
            for (let offset = 0; offset < ids.length; offset += CHUNK) {
                ...
                const statement = db.prepare(`SELECT event_id FROM events WHERE tenant_id = ? AND event_id IN (${placeholders})`);
                const rows = statement.all(tenantId, ...chunk);
                for (const row of rows)
                    found.add(EventId.unsafe(row.event_id));
            }
            return found;
        },
        async read(query) {
            const conditions = ['tenant_id = ?', 'session_id = ?'];
            const parameters = [query.tenantId, query.sessionId];
            ...
            const statement = db.prepare(`SELECT * FROM events WHERE ${conditions.join(' AND ')} ORDER BY chain_position ASC LIMIT ?`);
            const rows = statement.all(...parameters, limit + 1);
        ...
        async readSession(tenantId, sessionId) {
            const rows = selectSession.all(tenantId, sessionId);
            return rows.map(toTrustEvent);
        },
        async listSessions(tenantId, developerId, options) {
            const rows = selectSessions.all(tenantId, developerId);
        ...
        async eraseSubject(tenantId, developerId) {
            const sessions = db
                .prepare('SELECT DISTINCT session_id FROM events WHERE tenant_id = ? AND developer_id = ?')
                .all(tenantId, developerId);
            const result = db
...
function toTrustEvent(row) {
    return {
        eventId: EventId.unsafe(row.event_id),
        tenantId: TenantId.unsafe(row.tenant_id),
        sessionId: SessionId.unsafe(row.session_id),
        developerId: DeveloperId.unsafe(row.developer_id),
        assessmentId: row.assessment_id === null ? undefined : row.assessment_id,
        taskId: row.task_id === null ? undefined : row.task_id,
        type: row.type,
        layer: row.layer,
        ...
        redactedFields: JSON.parse(row.redacted_fields),
        integrity: {
            algorithm: 'sha256',
            hash: row.hash,
