...
import { DeveloperId, EVENT_SCHEMA_VERSION, EventId, EventSource, SessionId, TenantId, TrustEventType, describeEvent, sealEvent, unsafeEpochMs, verifyChain, } from '@scora/trust-core';
 ...
 */
export function makeEvent(chainPosition, previousHash, overrides = {}) {
...
export function makeChain(length, overrides = {}) {
    const events = [];
...
export function runStoreConformance(name, create) {
    describe(`${name} — EventStore conformance`, () => {
        ...
        it('appends and reads back in chain order', async () => {
            ...
            assert.deepEqual(stored.map((event) => event.chainPosition), [1, 2, 3, 4, 5]);
        });
        ...
        it('refuses a batch spanning two sessions', async () => {
            ...
            await assert.rejects(() => store.append([first, foreign]), /one tenant and session/, 'a cross-session batch cannot preserve per-session chain continuity');
        });
        ...
        it('isolates tenants', async () => {
            ...
            assert.equal((await store.existing(OTHER_TENANT, [events[0].eventId])).size, 0, 'one tenant must never learn what another holds');
        });
        it('filters a read by chain position range', async () => {
            ...
            assert.deepEqual(page.events.map((event) => event.chainPosition), [4, 5, 6]);
        });
