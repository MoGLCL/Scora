...
export function canonicalize(value, path = '') {
    ...
    if (value instanceof Date) {
        throw new CanonicalizationError('Date is not permitted; use an epoch-milliseconds number so the encoding is unambiguous', path);
    }
    const prototype = Object.getPrototypeOf(value);
    if (prototype !== Object.prototype && prototype !== null) {
        throw new CanonicalizationError(`Only plain objects can be canonicalized; received ${value.constructor?.name ?? 'unknown'}`, path);
    }
    const record = value;
    ...
    for (const key of keys) {
        ...
        if (entry === undefined) {
            throw new CanonicalizationError(`Property "${key}" is undefined; omit it or set it to null explicitly`, path);
        }
