...
import { LAYER_DEFINITIONS, ORDERED_LAYERS, TrustLayer } from "./layers.js";
import { EVENT_REGISTRY, EvidencePolarity, Sensitivity, canContributeToRisk, describeEvent, eventsForLayer, } from "./registry.js";
import { ALL_EVENT_TYPES, TrustEventType, isTrustEventType } from "./types.js";
import { PAYLOAD_SCHEMAS } from "../validation/payloads.js";
describe('event taxonomy', () => {
    ...
    it('assigns each event to a known layer', () => {
        const layers = new Set(Object.values(TrustLayer));
    ...
    it('throws on an unregistered type rather than returning undefined', () => {
        assert.throws(() => describeEvent('NOT_A_REAL_EVENT'));
    });
});
describe('scoring policy encoded in the registry', () => {
    it('never lets a risk-contributing event stand on its own', () => {
        const offenders = Object.values(EVENT_REGISTRY).filter((definition) => definition.polarity === EvidencePolarity.RISK_CONTRIBUTING && definition.sufficientAlone);
        assert.deepEqual(offenders.map((definition) => definition.type), [], 'a single risk signal must never be sufficient evidence');
    });
    it('treats editor assistance as neutral or supportive, never as inherent risk', () => {
        ...
        for (const definition of aiEvents) {
            assert.notEqual(definition.polarity, EvidencePolarity.RISK_CONTRIBUTING, `${definition.type} must not treat editor assistance as risk`);
        }
    });
    it('exposes no conversational assistant events, since the sandbox has no chat', () => {
        const forbidden = ALL_EVENT_TYPES.filter((type) => /ASK_AI|CHAT|PROMPT|EXPLANATION_REQUESTED|SUGGESTION_REQUESTED|GENERATED_CODE/.test(type));
        assert.deepEqual(forbidden, [], 'the sandbox is an editor with completions, not a code-generation chatbot');
    });
    it('has no payload field that could carry a developer prompt', () => {
        for (const definition of eventsForLayer(TrustLayer.AI_ASSISTANCE)) {
            ...
            assert.ok(result.error.some((issue) => issue.path === 'prompt' && issue.code === 'UNKNOWN_FIELD'), `${definition.type} must reject a prompt field outright`);
        }
    });
    ...
    it('treats consulting documentation as ordinary behaviour', () => {
        assert.equal(describeEvent(TrustEventType.EXTERNAL_RESOURCE_ACCESSED).polarity, EvidencePolarity.NEUTRAL);
    });
    it('counts modifying and testing AI output as supportive evidence', () => {
        assert.equal(describeEvent(TrustEventType.AI_SUGGESTION_MODIFIED).polarity, EvidencePolarity.SUPPORTIVE);
        assert.equal(describeEvent(TrustEventType.AI_SUGGESTION_TESTED).polarity, EvidencePolarity.SUPPORTIVE);
    });
    it('keeps the risk-contributing set small and deliberate', () => {
        const riskEvents = ALL_EVENT_TYPES.filter(canContributeToRisk);
        assert.deepEqual([...riskEvents].sort(), [
        ...
        ].sort(), 'adding a risk-contributing event is a policy decision and must be explicit');
    });
    ...
    it('marks interview answers and recordings as personal data', () => {
        assert.equal(describeEvent(TrustEventType.INTERVIEW_ANSWER_RECEIVED).sensitivity, Sensitivity.PERSONAL);
        assert.equal(describeEvent(TrustEventType.INTERVIEW_RECORDING_STORED).sensitivity, Sensitivity.PERSONAL);
    });
    it('routes feed problems to confidence rather than to risk', () => {
        for (const definition of eventsForLayer(TrustLayer.SYSTEM)) {
            assert.equal(definition.polarity, EvidencePolarity.CONFIDENCE_AFFECTING, 'losing telemetry says nothing about the developer');
        }
    });
});
describe('layer definitions', () => {
    ...
    it('requires corroboration for every layer that can produce adverse findings', () => {
        const adverseLayers = new Set(Object.values(EVENT_REGISTRY)
        ...
        for (const layer of adverseLayers) {
            assert.ok(LAYER_DEFINITIONS[layer].corroboratedBy.length > 0, `${layer} can raise risk and must name corroborating layers`);
        }
    });
    it('never names a layer as its own corroborator', () => {
        for (const definition of Object.values(LAYER_DEFINITIONS)) {
            assert.ok(!definition.corroboratedBy.includes(definition.layer), `${definition.layer} cannot corroborate itself`);
        }
    });
});
