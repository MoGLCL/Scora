...
import { v, type Validator } from './schema.ts';
...
const ExecutionOutcome = ['success', 'failure', 'error', 'timeout', 'cancelled'] as const;
...
const ENVIRONMENT_SCHEMAS = {
  [EnvironmentEventType.SESSION_STARTED]: v.object(
    {
      ...
      consentScopes: v.array(
        v.literalUnion(['assessment', 'external_monitoring', 'recording']),
        { maxItems: 8 },
      ),
  ...
  [EnvironmentEventType.DEVICE_CONTEXT_CHANGED]: v.object({
    changedFields: v.array(
      v.literalUnion(['user_agent', 'viewport_class', 'timezone', 'platform', 'screen_count']),
      { maxItems: 8 },
    ),
...
export const PAYLOAD_SCHEMAS: Readonly<Record<TrustEventType, Validator<unknown>>> = {
  ...SYSTEM_SCHEMAS,
  ...ENVIRONMENT_SCHEMAS,
  ...INTERACTION_SCHEMAS,
  ...TYPING_SCHEMAS,
...
} as unknown as Readonly<Record<TrustEventType, Validator<unknown>>>;

export function payloadSchemaFor(type: TrustEventType): Validator<unknown> {
...
export function eventTypesMissingSchemas(): readonly string[] {
