...
import { EventSource, type TrustEvent } from '../events/envelope.ts';
import { TrustLayer } from '../events/layers.ts';
...
import { ChainViolationCode, sealEvent, verifyChain, verifyEventHash } from './chain.ts';
...
function makeEvent(
  ...
  const unsealed: Omit<TrustEvent, 'integrity'> = {
...
function makeChain(length: number): TrustEvent[] {
...
describe('event sealing', () => {
  ...
  it('detects redaction metadata being altered after the fact', () => {
    const event = makeEvent(1, null);
    const tampered: TrustEvent = { ...event, redactedFields: ['payload.secret'] };
    assert.equal(verifyEventHash(tampered, sha256), false);
  });
...
describe('verifyChain', () => {
  ...
  it('rejects a first event that does not declare itself genesis', () => {
    ...
    assert.deepEqual(
      ...
      [ChainViolationCode.INVALID_GENESIS],
    );
  });
  ...
  it('flags two events claiming the same chain position', () => {
    ...
    assert.ok(
      result.violations.some(
        (violation) => violation.code === ChainViolationCode.CHAIN_POSITION_DUPLICATE,
      ),
      'only this engine assigns chain positions, so a collision cannot be an accident',
    );
  });
  ...
  it('refuses to verify events from two sessions as one chain', () => {
    ...
    assert.equal(result.intact, false);
    assert.ok(
      result.violations.some((violation) => violation.code === ChainViolationCode.SESSION_MIXING),
    );
  });
  ...
  it('reports every violation rather than stopping at the first', () => {
    ...
    const mismatches = result.violations.filter(
      (violation) => violation.code === ChainViolationCode.HASH_MISMATCH,
    );
