...
describe('canonicalize', () => {
  ...
  it('rejects undefined object properties instead of silently dropping them', () => {
    assert.throws(
      () => canonicalize({ present: 1, missing: undefined }),
      (error: unknown) =>
        error instanceof CanonicalizationError && error.path === '' && /missing/.test(error.message),
    );
  });
  ...
  it('rejects undefined inside arrays, which JSON.stringify would turn into null', () => {
    assert.throws(
      ...
      (error: unknown) => error instanceof CanonicalizationError && error.path === '[1]',
    );
  });
  ...
  it('reports a usable path for deeply nested failures', () => {
    assert.throws(
      ...
      (error: unknown) => error instanceof CanonicalizationError && error.path === 'a.b[0].c',
    );
  });
