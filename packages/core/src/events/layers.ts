...
export const LAYER_DEFINITIONS: Readonly<Record<TrustLayer, TrustLayerDefinition>> = {
  [TrustLayer.SYSTEM]: {
    layer: TrustLayer.SYSTEM,
    ordinal: 0,
    name: 'System & Feed Integrity',
    purpose:
      'Observations the engine makes about the telemetry feed itself: rejected events, sequence gaps, clock anomalies. Degrades Confidence rather than Trust.',
  ...
  [TrustLayer.ENVIRONMENT]: {
    ...
    purpose:
      'Integrity and reliability of the assessment environment: session lifecycle, sandbox connectivity, focus and visibility, interruptions, runtime integrity.',
  ...
  [TrustLayer.INTERACTION]: {
    ...
    purpose:
      'How the developer moves through the sandbox: files, navigation, search, terminal, panels, test runs. Behavioural evidence only.',
  ...
  [TrustLayer.TYPING]: {
    ...
    purpose:
      'How code is written and revised: rhythm, bursts, pauses, corrections, pastes, rewrites. Interpreted against the developer own baseline, never against a fixed threshold.',
  ...
  [TrustLayer.CODE_EVOLUTION]: {
    ...
    name: 'Code Evolution',
    purpose:
      'The whole trajectory of the code rather than the final submission: snapshots, diffs, refactors, growth curve, and when the significant changes happened.',
  ...
  [TrustLayer.RUNTIME]: {
    ...
    purpose:
      'How the developer engages with running code: executions, tests, errors, debugging cycles, fix attempts, recovery time, regressions.',
  ...
  [TrustLayer.EXTERNAL]: {
    ...
    purpose:
      'Use of resources outside the sandbox, where monitoring is lawful and disclosed. Documentation and search are normal professional behaviour; what matters is whether the developer adapted and understood what they found.',
  ...
  [TrustLayer.SKILL]: {
    ...
    purpose:
      'Reconciles claimed skills against assessment performance, observed behaviour, code evidence, runtime evidence and understanding evidence.',
  ...
  [TrustLayer.INTERVIEW]: {
    ...
    purpose:
      'Adaptive technical interview generated from the developer actual session. The strongest available test of ownership and understanding.',
  ...
  [TrustLayer.HUMAN_REVIEW]: {
    ...
    purpose:
      'Human adjudication over the full evidence package. Authoritative: a reviewer decision overrides the engine recommendation and is recorded immutably.',
...
export const ORDERED_LAYERS: readonly TrustLayer[] = Object.values(LAYER_DEFINITIONS)
  .slice()
