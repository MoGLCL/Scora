...
import { median, medianAbsoluteDeviation, percentile, recencyWeight, robustZScore, sampleConfidence, weightedMedian, } from "./statistics.js";
/**
...
export function observationFrom(extraction, meta) {
    ...
    const coverage = extraction.layers.length === 0
        ? 0
...
export function buildProfile(tenantId, developerId, observations, options) {
    ...
    for (const feature of TRACKED_FEATURES) {
        ...
        for (const observation of relevant) {
            ...
            weights.push(observation.coverage *
    ...
    return {
        ...
        firstObservedAt: (first?.observedAt ?? options.now),
        lastObservedAt: (last?.observedAt ?? options.now),
        dimensions,
 ...
 */
export function compare(feature, observed, profile, options = {}) {
    ...
    if (options.allowPopulationFallback === true && population !== undefined) {
        return {
            ...
            note: `${feature}: ${round(observed)} compared against the population median ` +
    ...
    return {
        ...
        note: `${feature}: ${round(observed)} observed. No established baseline exists for this ` +
            `developer, so no comparison is being made. This is not a finding.`,
    };
}
/** Compares every metric in a session against the profile. */
export function compareSession(observation, profile, options = {}) {
    return Object.entries(observation.metrics).map(([feature, value]) => compare(feature, value, profile, options));
}
/**
 ...
 */
export function notableDeviations(deviations, threshold = 3) {
    return deviations
        .filter((deviation) => deviation.basis === ComparisonBasis.SELF &&
        deviation.zScore !== null &&
        Math.abs(deviation.zScore) >= threshold)
        .sort((a, b) => Math.abs(b.zScore) - Math.abs(a.zScore));
}
export function buildPopulation(tenantId, profiles, cohort) {
    const dimensions = new Map();
    for (const feature of TRACKED_FEATURES) {
...
function describe(feature, observed, expected, zScore, basis, samples) {
    ...
    if (magnitude < 1.5)
        return `${head} — within their normal range.`;
    if (magnitude < 3) {
        return `${head} — somewhat outside their usual range (z=${round(zScore)}), which is ordinary session-to-session variation.`;
    }
    return (`${head} — well outside their usual range (z=${round(zScore)}). Worth understanding, ` +
