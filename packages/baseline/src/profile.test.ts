...
import { ComparisonBasis, type SessionObservation } from './contract.ts';
import {
...
import { inMemoryBaselineStore } from './store.ts';
...
function observation(
  ...
  overrides: Partial<Omit<SessionObservation, 'coverage'>> & { coverage?: number } = {},
): SessionObservation {
...
function stableHistory(
  ...
  developerId: typeof DEV = DEV,
) {
  return Array.from({ length: count }, (_, index) =>
    observation(index, { 'typing.rate_cpm': rate + (index % 2 === 0 ? 5 : -5), ...extra }, {
      developerId,
    }),
  );
}
...
describe('profile construction', () => {
  ...
  it('ignores other developers observations', () => {
    const mixed = [
      ...
      ...Array.from({ length: 20 }, (_, i) =>
        observation(i, { 'typing.rate_cpm': 2_000 }, { developerId: OTHER }),
      ),
    ];
  ...
  it('downweights sessions with poor telemetry', () => {
    ...
    assert.ok(
      profile.dimensions.get('typing.rate_cpm')!.median < 400,
      'a barely-observed session must not define the baseline',
    );
  });
  ...
  it('tracks no risk-contributing feature', () => {
    ...
    for (const feature of TRACKED_FEATURES) {
      assert.doesNotMatch(
        feature,
        /unexplained|unadapted|dependency_index|integrity_violation|device_context/,
        `${feature} would build a history of suspicion`,
      );
    }
  });
...
describe('comparison fairness', () => {
  ...
  it('never reports a population comparison as notable', () => {
    ...
    assert.deepEqual(
      notableDeviations([deviation]),
      [],
      'differing from other people is not a finding about a person',
    );
  });
});
...
describe('baseline store', () => {
  ...
  it('isolates tenants', async () => {
    ...
    assert.equal(
      await store.load(otherTenant, DEV),
      null,
      'one tenant must never be served another tenant data',
    );
  });
});
