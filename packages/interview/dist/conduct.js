...
import { AnswerOutcome, Difficulty, InterviewVerdict, NO_EXAMINER, QuestionTopic, } from "./contract.js";
import { adjustDifficulty } from "./plan.js";
/**
...
export async function conductInterview(plan, options) {
    ...
    for (const planned of plan) {
        ...
        if (adjusted.next !== difficulty)
            adjustments.push(adjusted.reason);
...
export function normaliseGrade(grade) {
    ...
    if (graderConfidence < MINIMUM_GRADER_CONFIDENCE) {
        return {
            ...
            rationale: `The grader reported ${graderConfidence.toFixed(2)} confidence, below the ` +
    ...
    return {
        ...
        outcome: consistency >= CONSISTENT_THRESHOLD
...
function summarise(exchanges, options) {
    const graded = exchanges
        .map((exchange) => exchange.grade)
        .filter((grade) => grade !== null && isGraded(grade));
    const base = {
    ...
    const ownership = clampUnit(graded.reduce((sum, grade) => sum + grade.consistencyWithCode, 0) / graded.length);
    const inconsistent = graded.filter((grade) => grade.outcome === AnswerOutcome.INCONSISTENT).length;
...
function summaryOf(verdict, graded, asked, inconsistent) {
    switch (verdict) {
        ...
        case InterviewVerdict.EXPLANATION_INCONSISTENT:
            return (`${String(inconsistent)} of ${String(graded)} graded answers described something other than what the session shows. ` +
 ...
 */
function confidenceOf(graded, asked) {
    if (graded.length === 0)
        return clampUnit(0);
...
function limitationsOf(exchanges, graded) {
    const limitations = [];
    const unanswered = exchanges.filter((exchange) => exchange.answer === null).length;
    const ungraded = exchanges.filter((exchange) => exchange.grade !== null && exchange.grade.outcome === AnswerOutcome.UNGRADED).length;
    if (graded.length < 4) {
        limitations.push(`Only ${String(graded.length)} answers were graded. Read the individual exchanges rather than the aggregate.`);
    }
    if (unanswered > 0) {
        limitations.push(`${String(unanswered)} question(s) went unanswered. An unanswered question carries no penalty and no information.`);
    }
    if (ungraded > 0) {
        limitations.push(`${String(ungraded)} answer(s) were discarded because the grader was not confident enough to score them.`);
    }
    limitations.push('Interview performance reflects the ability to explain under time pressure, which is not the same as the ability to write code. ' +
        'Treat a weak interview as a prompt for human review, never as a finding on its own.');
    return limitations;
}
function uncoveredTopics(exchanges) {
...
function notAnswered(questionId) {
    return {
        questionId,
 ...
 */
export function interviewStanding(result) {
