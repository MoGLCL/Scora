 ...
 */
export function planInterview(events, options = {}) {
 ...
 */
function unverifiedAssistedRegions(events) {
    ...
    for (const event of events) {
        ...
        if (event.type === AiAssistanceEventType.AI_SUGGESTION_MODIFIED ||
 ...
 */
function resolvedErrors(events) {
    ...
    return resolved.map((event) => {
        const payload = event.payload;
        const signature = typeof payload.errorSignatureHash === 'string' ? payload.errorSignatureHash : '';
        const origin = observed.find((candidate) => {
            const observedPayload = candidate.payload;
 ...
 */
function unresolvedSkillClaims(assessment) {
    if (assessment === undefined)
        return [];
    return assessment.findings
        .filter((finding) => finding.verdict === SkillVerdict.NOT_EXERCISED ||
 ...
 */
export function adjustDifficulty(current, consistency, graderConfidence) {
...
function DIFFICULTY_AT(ordinal) {
    return ([Difficulty.EASY, Difficulty.MEDIUM, Difficulty.HARD, Difficulty.EXPERT][ordinal] ??
        Difficulty.MEDIUM);
}
