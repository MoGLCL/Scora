...
import { AiAssistanceEventType, CodeEvolutionEventType, RuntimeEventType, SkillEventType, TrustLayer, unsafeEpochMs, } from '@scora/trust-core';
import { T0, buildSession, payloads } from '@scora/trust-features/testing';
import { assessSkills } from '@scora/trust-skills';
import { AnswerOutcome, Difficulty, InterviewVerdict, QuestionTopic, } from "./contract.js";
import { MAX_QUESTIONS, adjustDifficulty, planInterview, } from "./plan.js";
import { CONSISTENT_THRESHOLD, conductInterview, interviewStanding, normaliseGrade, } from "./conduct.js";
/**
...
function claimed(skillId, name, level, at) {
    return {
...
function honestExaminer(consistency, confidence = 0.9, specificity = 0.8, depth = 0.7) {
    return {
...
async function interview(planQuestions, examiner, answers) {
...
describe('the interview', () => {
    ...
    it('is INCONCLUSIVE when too few answers could be graded', async () => {
        const result = await interview(plan([
            taskStarted([TS], 0),
            accepted(12, 60_000),
            errorObserved('boom', 120_000),
            errorResolvedEvent('boom', 180_000),
            taskSubmitted(300_000),
        ]), honestExaminer(0.9), async () => null);
    ...
    it('demonstrates ownership when explanations match the artefact', async () => {
        const result = await interview(plan([
            ...
            dependency(200_000),
            taskSubmitted(300_000),
        ]), honestExaminer(0.9), async () => WORDY_ANSWER);
    ...
    it('flags a fluent explanation of code that is not theirs', async () => {
        const result = await interview(plan([
            ...
            dependency(200_000),
            taskSubmitted(300_000),
        ]), honestExaminer(0.1, 0.95), async () => WORDY_ANSWER);
    ...
    it('applies the requested starting difficulty and adapts forward', async () => {
        ...
        assert.ok(result.exchanges.some((exchange) => exchange.question.difficulty !== Difficulty.EASY), 'expected the difficulty to adapt upward');
    });
    it('reports topics the interview never reached', async () => {
        const result = await interview(plan([taskStarted([TS], 0), accepted(12, 60_000), taskSubmitted(300_000)]), honestExaminer(0.9), async () => WORDY_ANSWER);
    ...
    it('never claims certainty, however well it went', async () => {
        const result = await interview(plan([
            ...
            dependency(200_000),
            taskSubmitted(300_000),
        ]), honestExaminer(1, 1), async () => WORDY_ANSWER);
