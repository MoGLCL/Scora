...
import { FeaturePolarity } from "./contract.js";
import { EXTRACTED_LAYERS, FEATURE_DEFINITIONS, extractFeatures } from "./extract.js";
import { TEST_CONTEXT, buildSession, hexHash, payloads } from "./testing.js";
/**
...
describe('extraction contract', () => {
    ...
    it('is deterministic', () => {
        ...
        assert.deepEqual(first.all.map((f) => [f.name, f.value, f.confidence]), second.all.map((f) => [f.name, f.value, f.confidence]));
    });
});
    ...
    it('trips no risk-contributing feature', () => {
        ...
        for (const feature of result.all) {
            if (feature.polarity === FeaturePolarity.RISK_CONTRIBUTING) {
                assert.ok(feature.value === null || feature.value === 0, `${feature.name} fired at ${feature.value} for an honest solo session`);
            }
        }
    });
...
describe('archetype: human with documentation', () => {
    ...
    it('treats documentation use as supportive, never as risk', () => {
        ...
        assert.equal(result.featuresByName.get('external.visit_count').polarity, FeaturePolarity.NEUTRAL);
    });
    ...
    it('trips no risk-contributing feature', () => {
        const result = extract(specs);
        const fired = result.all.filter((f) => f.polarity === FeaturePolarity.RISK_CONTRIBUTING && (f.value ?? 0) > 0);
...
describe('false-positive guards', () => {
    ...
    it('does not penalise accepting a large completion that was then reworked', () => {
        ...
        assert.equal(value(result, 'assist.dependency_index'), 0, 'instant acceptance followed by substantial rework is not dependency');
    });
    ...
    it('lowers confidence when comparing against the session instead of history', () => {
        const withoutHistory = extractFeatures(buildSession([
            sessionStart,
        ...
        const withHistory = extractFeatures(buildSession([
            sessionStart,
            { type: TrustEventType.TEXT_INSERTED, at: 1_000, payload: payloads.insert(40, 2) },
            { type: TrustEventType.LARGE_INSERTION, at: 5_000, payload: payloads.largeInsertion(1_200, 40, 'paste') },
        ]), {
...
describe('policy invariants', () => {
    ...
    it('gives every risk-contributing feature an interpretation demanding corroboration', () => {
        for (const definition of FEATURE_DEFINITIONS.values()) {
            if (definition.polarity !== FeaturePolarity.RISK_CONTRIBUTING)
                continue;
            assert.match(definition.interpretation, /corroborat|cluster|alone|conjunctive|not a (conclusion|verdict|finding)/i, `${definition.name} must state that it cannot stand alone`);
        }
    });
    it('has no risk-contributing feature in the interaction layer', () => {
        for (const definition of FEATURE_DEFINITIONS.values()) {
            if (definition.layer !== TrustLayer.INTERACTION)
                continue;
    ...
    it('keeps raw editor-assistance usage neutral', () => {
        for (const name of ['assist.acceptance_rate', 'assist.assisted_character_ratio', 'assist.suggestions_shown']) {
            assert.equal(FEATURE_DEFINITIONS.get(name).polarity, FeaturePolarity.NEUTRAL, `${name} must not imply that using the editor is adverse`);
        }
    });
    it('keeps external access itself neutral', () => {
        ...
        assert.equal(FEATURE_DEFINITIONS.get('external.ai_tool_visit_count').polarity, FeaturePolarity.NEUTRAL);
    });
