...
import { coefficientOfVariation, collectField, evidenceIds, eventsOfType, fmt, fmtDuration, makeFeature, median, numberField, ratio, sampleConfidence, stringField, sumField, } from "../window.js";
/**
...
export const TYPING_FEATURES = [
    ...
    },
    {
        ...
        interpretation: 'High values are supportive. Low values are NOT adverse on their own — a developer assembling code from their own earlier work, or accepting editor completions, legitimately types little.',
    },
    {
        ...
        interpretation: 'Human typing is irregular: people pause to think mid-line. Near-zero variability suggests synthetic input rather than a fast human. High variability is normal and supportive.',
    },
    {
        ...
        interpretation: 'Self-correction is evidence of authorship: people who write code fix typos, rename things and change their minds. Code that arrives already perfect was authored elsewhere.',
    },
    {
        ...
        interpretation: 'Revisiting and reworking existing code requires understanding it. Among the more reliable ownership signals in this layer.',
    },
    {
        ...
        interpretation: 'Explicitly NEUTRAL. Pasting is ubiquitous in real development. This feature exists to be combined with paste origin and subsequent modification, never to stand alone.',
    },
    {
        ...
        interpretation: 'Moving your own code around is ordinary refactoring and is supportive, not suspicious. This feature separates that from importation.',
    },
    {
        ...
        description: 'Share of characters arriving in bulk insertions from outside the workspace, unaccounted for by editor completions.',
        inputs: [TrustEventType.LARGE_INSERTION, TrustEventType.CODE_PASTE],
        calculation: 'characters from externally-originated large insertions / all inserted characters',
    ...
    },
    {
        ...
        calculation: 'largest insertion characters / median insertion size, taken from the developer baseline when available and from this session otherwise',
        interpretation: 'Relative by construction. A developer who always works in large blocks has a low multiple even for big insertions; one who normally types line by line does not. Without history the session median is used and confidence is reduced accordingly.',
    },
    {
        ...
        interpretation: 'Context for Layer 06 correlation. A long pause before a large insertion is equally consistent with reading documentation, thinking, or being away — it is a question to ask, not an answer.',
    },
    {
        ...
        interpretation: 'Null when no history exists, which is the correct answer for a first session — it must never default to a population average, because that would penalise anyone atypical.',
    },
];
...
export function extractTypingFeatures(window, context) {
    ...
    const externalBulk = largeInsertions.filter((event) => {
        ...
        const originPaste = pastes.find((paste) => Math.abs(paste.occurredAtNormalized - event.occurredAtNormalized) < 2000 &&
    ...
    const largestInsertion = Math.max(0, ...largeInsertions.map((event) => numberField(event, 'charactersAdded') ?? 0));
    const baselineMultiple = referenceMedian !== null && referenceMedian > 0 && largestInsertion > 0
    ...
    features.push(makeFeature({
        ...
        confidence: clampUnit(sampleConfidence(largeInsertions.length) * (baselineMedian === null ? 0.5 : 1)),
        evidence: evidenceIds(largeInsertions, inserts),
        note: baselineMultiple === null
    ...
    features.push(makeFeature({
        ...
        note: preInsertionGaps.length === 0
    ...
    features.push(makeFeature({
        ...
        kind: FeatureKind.INDEX,
        polarity: FeaturePolarity.CONTEXTUAL,
        value: sessionRate !== null && baselineRate !== null && baselineRate > 0
            ? sessionRate / baselineRate
            : null,
        sampleSize: burstRates.length,
        evidence: evidenceIds(bursts),
        note: baselineRate === null
