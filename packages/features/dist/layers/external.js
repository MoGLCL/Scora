...
import { collectField, evidenceIds, eventsOfType, fmtDuration, makeFeature, median, numberField, ratio, stringField, } from "../window.js";
/**
...
export const EXTERNAL_FEATURES = [
    {
        ...
        interpretation: 'Explicitly NEUTRAL. Looking things up is normal engineering. This is a count, not an accusation.',
    },
    {
        ...
        interpretation: 'Reading primary sources is a positive professional habit. Treated as supportive, in direct contrast to systems that flag any external tab.',
    },
    {
        ...
        interpretation: 'Cross-referencing several kinds of source suggests the developer was reasoning about the problem rather than looking for something to copy.',
    },
    {
        ...
        interpretation: 'Reading takes time; grabbing a snippet does not. Neither is adverse alone — a developer who already knows an API needs one glance at a signature.',
    },
    {
        ...
        interpretation: 'Going back to a reference indicates working through it rather than lifting from it. Supportive.',
    },
    {
        ...
        interpretation: 'NEUTRAL by policy. AI assistance is permitted; SCORA measures whether the developer understands and owns the result, not whether they had help. Also incomplete by nature — a second device is undetectable — so this must never be treated as a reliable AI detector.',
    },
    {
        ...
        calculation: 'mean(adaptationRatio) where reported, else imports followed by a revising diff to the same file / all imports',
        interpretation: 'The distinction that matters in this layer. Taking an example and fitting it to your codebase demonstrates understanding; pasting it unchanged does not. High values are strongly mitigating.',
    },
    {
        ...
        description: 'Externally-correlated imports that were never adapted and never verified afterwards.',
        inputs: [
        ...
        calculation: 'count of EXTERNAL_ORIGIN_IMPORT events with low adaptation, no later revising diff to the same file, and no later test run or execution',
        interpretation: 'The single risk-contributing feature in this layer, and deliberately conjunctive: it requires an external correlation AND absent adaptation AND absent verification. Any one of those alone is ordinary behaviour. Even satisfied, it must be corroborated by Layer 08 or 09 before it affects any decision.',
    },
    {
        ...
        interpretation: 'Short gaps mean less time to read and think. Context for the interview, which is where understanding is actually established.',
    },
];
/** Below this, an import is treated as substantially unmodified. */
const LOW_ADAPTATION_THRESHOLD = 0.15;
export function extractExternalFeatures(window, _context) {
    ...
    const documentationVisits = visits.filter((event) => stringField(event, 'category') === 'official_documentation');
    features.push(makeFeature({
        name: 'external.documentation_ratio',
        layer: LAYER,
        polarity: FeaturePolarity.SUPPORTIVE,
    ...
    const categories = new Set(visits
    ...
    const wasAdapted = (event) => {
        const reported = numberField(event, 'adaptationRatio');
        if (reported !== null && reported >= LOW_ADAPTATION_THRESHOLD)
            return true;
        return diffs.some((diff) => diff.occurredAtNormalized > event.occurredAtNormalized &&
    ...
    const verifiedAfter = (event) => [...testRuns, ...executions].some((run) => run.occurredAtNormalized > event.occurredAtNormalized);
