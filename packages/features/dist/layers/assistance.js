...
import { collectField, evidenceIds, eventsOfType, fmt, fmtDuration, makeFeature, mean, median, numberField, ratio, stringField, sumField, } from "../window.js";
/**
...
export const ASSISTANCE_FEATURES = [
    {
        ...
        interpretation: 'Purely descriptive. The editor decides what to show; the developer does not control this and must never be judged on it.',
    },
    {
        ...
        interpretation: 'Explicitly NEUTRAL. A high acceptance rate means the completions were good, which is a property of the editor. It is not a measure of the developer.',
    },
    {
        ...
        interpretation: 'Deciding a suggestion is wrong requires understanding what it would do. Discrimination is a positive signal.',
    },
    {
        ...
        interpretation: 'Taking half a completion and writing the rest yourself is strong evidence of reading it. Among the better ownership signals in this layer.',
    },
    {
        ...
        interpretation: 'The central quantity separating assisted from dependent use. Code that was accepted and then reshaped was engaged with.',
    },
    {
        ...
        kind: FeatureKind.RATIO,
        polarity: FeaturePolarity.SUPPORTIVE,
        description: 'Share of accepted suggestions that were afterwards modified, deleted or tested.',
        inputs: [
            TrustEventType.AI_SUGGESTION_ACCEPTED,
        ...
        calculation: 'distinct suggestionIds appearing in a modified, deleted or tested event / distinct accepted suggestionIds',
        interpretation: 'The headline feature of this layer. Doing something with a suggestion after taking it — even deleting it — shows the developer was evaluating rather than accumulating.',
    },
    {
        ...
        interpretation: 'Checking that accepted code actually works is basic professional discipline and is credited as such.',
    },
    {
        ...
        interpretation: 'Very short times on single-token completions are entirely normal — that is how autocomplete works. The same speed on a multi-line block means it was not read. Interpretation therefore depends on the size of what was accepted.',
    },
    {
        ...
        interpretation: 'NEUTRAL by policy. This is the number a naive system would misuse as an "AI percentage". It describes tooling usage, not trustworthiness, and is reported only to give the engagement features a denominator.',
    },
    {
        ...
        description: 'Degree to which large accepted suggestions were taken instantly and never engaged with.',
        inputs: [
            TrustEventType.AI_SUGGESTION_ACCEPTED,
            TrustEventType.AI_SUGGESTION_MODIFIED,
            TrustEventType.AI_SUGGESTION_TESTED,
        ],
        calculation: 'share of accepted suggestions that were multi-line, accepted below a deliberation floor, and never modified, deleted or tested',
...
export function extractAssistanceFeatures(window, _context) {
    ...
    features.push(makeFeature({
        ...
        note: modificationRatios.length === 0
    ...
    const acceptedIds = new Set(accepted
        ...
        .filter((id) => id !== null));
    const engagedIds = new Set([...modified, ...deleted, ...tested]
        .map((event) => stringField(event, 'suggestionId'))
        .filter((id) => id !== null));
    ...
    const testedIds = new Set(tested
        .map((event) => stringField(event, 'suggestionId'))
        .filter((id) => id !== null));
    ...
    const assistedChars = sumField(accepted, 'charactersAccepted') +
    ...
    features.push(makeFeature({
        ...
        note: dependentAcceptances.length === 0
