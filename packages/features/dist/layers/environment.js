...
import { coverageConfidence, evidenceIds, eventsOfType, fmt, fmtDuration, makeFeature, median, numberField, sampleConfidence, sumField, } from "../window.js";
/**
...
export const ENVIRONMENT_FEATURES = [
    {
        ...
        interpretation: 'Low coverage means the engine saw less than it should have. It lowers Confidence in every other layer and never raises Risk on its own — a flaky network says nothing about a developer.',
    },
    {
        ...
        interpretation: 'Long gaps mean code may have changed unobserved. That warrants a verification challenge or interview question, not an adverse conclusion.',
    },
    {
        ...
        interpretation: 'A fragmented session is harder to reason about. Reported so reviewers can see it, not to penalise the developer for an unstable connection.',
    },
    {
        ...
        interpretation: 'Explicitly NEUTRAL. Leaving the window is ordinary developer behaviour. This feature exists to give context to other layers, never to suggest wrongdoing by itself.',
    },
    {
        ...
        interpretation: 'Descriptive context for correlating with Layer 06. High values alone mean nothing: some people work across two monitors.',
    },
    {
        ...
        interpretation: 'One of the few genuinely adverse signals, because it targets the measurement apparatus rather than the work. Still requires corroboration: detectors produce false positives against browser extensions and accessibility tools.',
    },
    {
        ...
        interpretation: 'Could indicate a different person taking over, but far more often means a browser update, a resized window or a monitor being connected. Never sufficient alone.',
    },
    {
        ...
        interpretation: 'All timing-derived features in Layers 03 and 05 depend on this. An unstable clock lowers their confidence rather than invalidating them.',
    },
];
export function extractEnvironmentFeatures(window, _context) {
    ...
    const features = [];
    const coverage = window.durationMs > 0 ? Math.max(0, 1 - totalGapMs / window.durationMs) : null;
    ...
    const focusRetention = window.durationMs > 0 ? Math.max(0, 1 - awayMs / window.durationMs) : null;
    features.push(makeFeature({
        name: 'env.focus_retention',
        layer: LAYER,
        polarity: FeaturePolarity.NEUTRAL,
    ...
    features.push(makeFeature({
        ...
        confidence: clampUnit(violations.length === 0
            ? 1
