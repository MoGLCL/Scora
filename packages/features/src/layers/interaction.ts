...
export const INTERACTION_FEATURES: readonly FeatureDefinition[] = [
  {
    ...
    interpretation:
      'Reading the surrounding code before changing it is what an experienced developer does. Low values are not adverse — a single-file task needs no exploration.',
  },
  {
    ...
    interpretation:
      'Using go-to-definition requires knowing what you are looking for. Strong corroborating evidence of genuine engagement with unfamiliar code.',
  },
  {
    ...
    interpretation:
      'Searching indicates orientation within the codebase. Neutral-to-positive; its absence proves nothing.',
  },
  {
    ...
    interpretation:
      'A developer who reads before and while writing is behaving normally. Very low values are worth noting only alongside other signals.',
  },
  {
    ...
    interpretation:
      'Running tooling directly indicates comfort with the environment. Absence is neutral — many tasks need no terminal.',
  },
  {
    ...
    interpretation:
      'Choosing to verify your own work, unprompted, is a strong ownership signal.',
  },
  {
    ...
    interpretation:
      'Explicitly NEUTRAL. Thinking, reading a spec and taking breaks all register as idle. Long idles before correct code are as consistent with careful thought as with anything else.',
  },
  {
    ...
    interpretation:
      'Only meaningful in a cluster: a long idle immediately followed by a large unmodified insertion is worth a question. The same idle followed by incremental typing is not.',
  },
  {
    ...
    interpretation:
      'Descriptive baseline material. Working styles vary enormously; this exists to compare a developer against themselves over time.',
  },
];
...
export function extractInteractionFeatures(
  ...
  const verificationCommands = commands.filter((event) => {
    ...
    if (!VERIFICATION_VERBS.has(verb)) return false;
