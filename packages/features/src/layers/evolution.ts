...
export const CODE_EVOLUTION_FEATURES: readonly FeatureDefinition[] = [
  {
    ...
    interpretation:
      'Determines how finely the evolution can be reconstructed. Few snapshots mean weaker claims about how the code developed, in either direction.',
  },
  {
    ...
    interpretation:
      'Many small changes are the signature of code being built rather than delivered. Few large ones are not adverse but do warrant corroboration.',
  },
  {
    ...
    interpretation:
      'Smooth growth is characteristic of incremental development. Lumpy growth is common and legitimate; it simply carries less independent weight.',
  },
  {
    ...
    interpretation:
      'A high share means most of the solution appeared at once. Only meaningful alongside what followed: substantial revision and testing afterwards make it unremarkable.',
  },
  {
    ...
    interpretation:
      'Going back and changing your own code requires understanding it. Purely additive sessions are compatible with transcription and merit a closer look at Layers 05 and 09.',
  },
  {
    ...
    interpretation:
      'Refactoring is difficult to perform without understanding what the code does. One of the strongest ownership signals available before the interview.',
  },
  {
    ...
    calculation:
      'bulk insertions followed by a diff touching the same file within the session / all bulk insertions',
    interpretation:
      'The mitigating counterpart to typing.unexplained_insertion_ratio. Code that arrived whole and was then reworked was engaged with; code that arrived whole and was never touched again was not.',
  },
  {
    ...
    interpretation:
      'Descriptive. Fast completion is a sign of competence at least as often as anything else.',
  },
  {
...
export function extractCodeEvolutionFeatures(
  ...
  const revisedInsertions = largeInsertions.filter((insertion) => {
    ...
    return diffs.some(
      (diff) =>
        diff.occurredAtNormalized > insertion.occurredAtNormalized &&
  ...
  const changeEvents = [...diffs, ...saves].sort(
    (a, b) => a.occurredAtNormalized - b.occurredAtNormalized,
  );
