...
import { DeveloperId, EVENT_SCHEMA_VERSION, EventId, HumanReviewEventType, SessionId, TrustEventType, unsafeEpochMs, } from '@scora/trust-core';
import { DEV, OTHER_DEV, OTHER_TENANT, SESSION, T0, TASK, TENANT, adminPrincipal, developerPrincipal, harness, reviewerPrincipal, } from '@scora/trust-api/testing';
...
import { MINIMUM_RATIONALE_LENGTH, assessReviewQuality, reconstructReview } from "./audit.js";
import { buildReviewRoutes, impliedDecision } from "./routes.js";
import { inMemoryReviewIndex } from "./queue.js";
/**
...
const ENOUGH_REASON = 'The submitted solution reproduces a private internal helper verbatim, and the ' +
...
async function seedSession(test, sessionId = SESSION, developerId = DEV) {
...
async function openReview(test, overrides = {}) {
...
describe('opening a review', () => {
    ...
    it('is idempotent, so a retried assignment does not read as a reassignment', async () => {
        ...
        const assignments = events.filter((event) => event.type === HumanReviewEventType.REVIEW_ASSIGNED);
...
describe('the decision', () => {
    it('refuses an adverse decision with no written reason', async () => {
        ...
        const events = await test.store.readSession(TENANT, SESSION);
        assert.equal(events.filter((event) => event.type === HumanReviewEventType.REVIEW_DECISION_RECORDED).length, 0, 'a refused decision must not reach the log');
    });
    ...
    it('keeps the hash of the reason and never the reason itself', async () => {
        ...
        const events = await test.store.readSession(TENANT, SESSION);
        const decision = events.find((event) => event.type === HumanReviewEventType.REVIEW_DECISION_RECORDED);
        ...
        assert.match(String(decision.payload['rationaleHash']), /^[0-9a-f]{64}$/);
        assert.equal(decision.payload['rationaleLength'], ENOUGH_REASON.length);
    });
    it('is immutable: a second decision is refused, not silently applied', async () => {
        ...
        const events = await test.store.readSession(TENANT, SESSION);
        const decisions = events.filter((event) => event.type === HumanReviewEventType.REVIEW_DECISION_RECORDED);
        assert.equal(decisions.length, 1);
        assert.equal(decisions[0].payload['decision'], ReviewDecision.APPROVE);
    });
...
describe('overriding the engine', () => {
    it('records the disagreement without the reviewer having to declare it', async () => {
        ...
        assert.equal(impliedDecision(RECOMMENDATION.HUMAN_REVIEW_REQUIRED), ReviewDecision.REQUEST_REVIEW);
    });
    it('has no mapping that produces REJECT — only a human may reject', () => {
        for (const recommendation of Object.values(RECOMMENDATION)) {
            assert.notEqual(impliedDecision(recommendation), ReviewDecision.REJECT, `${recommendation} must not imply a rejection`);
        }
    });
    it('records nothing when the engine had no position to contradict', async () => {
        ...
        const events = await test.store.readSession(TENANT, SESSION);
        const overrides = events.filter((event) => event.type === HumanReviewEventType.RECOMMENDATION_OVERRIDDEN);
...
describe('the queue', () => {
    it('puts the most urgent work first and the oldest first within a priority', async () => {
        ...
        const body = response.body;
        assert.deepEqual(body.reviews.map((entry) => entry.reviewId), ['rev_002', 'rev_001', 'rev_003']);
    });
    it('drops a review from the queue once it is decided', async () => {
        ...
        assert.deepEqual(pending.body.reviews, []);
