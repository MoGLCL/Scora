...
export function isAdverse(decision: ReviewDecision): boolean {
 ...
 * assessment notes it.
 */
export function reconstructReviews(events: readonly TrustEvent[]): readonly ReviewRecord[] {
  ...
  for (const event of events) {
    ...
    switch (event.type) {
      ...
      case HumanReviewEventType.REVIEW_OPENED: {
        ...
        if (record.openedAt === null) record.openedAt = event.occurredAt;
        if (record.assignedTo === null) record.assignedTo = stringOr(payload['reviewerId'], null);
        break;
      }
      ...
      case HumanReviewEventType.REVIEW_EVIDENCE_ACCESSED: {
        ...
        record.accesses.push({
          ...
          evidenceKind: kind,
          viewDurationMs:
            typeof payload['viewDurationMs'] === 'number' ? payload['viewDurationMs'] : null,
        });
        break;
      }
      ...
      case HumanReviewEventType.REVIEW_DECISION_RECORDED: {
        ...
        record.decision = {
          ...
          decision,
          rationaleLength:
            typeof payload['rationaleLength'] === 'number' ? payload['rationaleLength'] : 0,
          rationaleHash: stringOr(payload['rationaleHash'], '') ?? '',
          reviewDurationMs:
            typeof payload['reviewDurationMs'] === 'number' ? payload['reviewDurationMs'] : 0,
        };
        break;
      }
      ...
      case HumanReviewEventType.RECOMMENDATION_OVERRIDDEN: {
        ...
        record.override = {
          ...
          humanDecision: human,
          engineTrustScore:
            typeof payload['engineTrustScore'] === 'number' ? payload['engineTrustScore'] : 0,
          rationaleHash: stringOr(payload['rationaleHash'], '') ?? '',
        };
        break;
      }
      ...
      case HumanReviewEventType.RESULT_RELEASED: {
        record.release = {
          ...
          scheduledFor:
            typeof payload['scheduledFor'] === 'number' ? payload['scheduledFor'] : null,
        };
        break;
      }
 ...
 */
export function assessReviewQuality(record: ReviewRecord): ReviewQuality {
  ...
  const rationaleRecorded =
    decision !== null && decision.rationaleLength >= MINIMUM_RATIONALE_LENGTH;
  ...
  if (kinds.size === 0) {
    concerns.push(
      'A decision was recorded without any evidence being opened. The reviewer may have ' +
  ...
  if (duration !== null && duration < MINIMUM_REVIEW_DURATION_MS) {
    concerns.push(
      `The decision was recorded ${String(Math.round(duration / 1000))}s after the package was opened, ` +
  ...
  if (isAdverse(decision.decision) && !rationaleRecorded) {
    concerns.push(
      'An adverse decision was recorded without a written rationale. A developer who is ' +
  ...
  if (isAdverse(decision.decision) && !kinds.has(EvidenceKind.FINAL_CODE)) {
    concerns.push(
      'An adverse decision was recorded without opening the submitted code. Behavioural ' +
  ...
  if (record.assignedTo !== null && decision.reviewerId !== record.assignedTo) {
    concerns.push(
      `The decision was recorded by ${decision.reviewerId}, who is not the assigned reviewer ` +
...
interface Mutable {
  reviewId: string;
  tenantId: ReviewRecord['tenantId'];
  sessionId: SessionId;
  developerId: DeveloperId;
  assignedTo: string | null;
  assignedBy: string | null;
  priority: ReviewPriority;
  assignedAt: EpochMs | null;
...
function blank(reviewId: string, event: TrustEvent): Mutable {
  return {
    reviewId,
...
function finalise(record: Mutable): ReviewRecord {
  return {
    ...record,
...
function stateOf(record: Mutable): ReviewState {
  if (record.release !== null) return ReviewState.RELEASED;
  if (record.decision !== null) return ReviewState.DECIDED;
  if (record.openedAt !== null || record.accesses.length > 0) return ReviewState.IN_PROGRESS;
...
function decisionOf(value: unknown): ReviewDecision | null {
...
function evidenceKindOf(value: unknown): EvidenceKind | null {
  return typeof value === 'string' &&
...
function priorityOf(value: unknown): ReviewPriority {
  return typeof value === 'string' &&
...
function channelOf(value: unknown): ReleaseRecord['channel'] {
