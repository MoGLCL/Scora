 ...
 */
export function inMemoryReviewIndex(): ReviewIndex {
  const byTenant = new Map<TenantId, Map<string, SessionId>>();

  return {
    async note(tenantId, reviewId, sessionId) {
      const reviews = byTenant.get(tenantId) ?? new Map<string, SessionId>();
