 ...
 */
function outcome(met, degree, reason, evidence, indeterminate = false) {
    return {
...
function ramp(value, start, full) {
    if (full === start)
        return value >= full ? 1 : 0;
    return clampUnit((value - start) / (full - start));
}
const UNVERIFIED_IMPORT = {
    id: 'unverified_external_import',
    title: 'Externally-sourced code taken without adaptation or verification',
    meaning: 'Code correlated with an external resource entered the workspace, was not adapted to the surrounding codebase, and was never executed or tested before submission.',
    minimumConditions: 3,
    ...
    conditions: [
        {
            ...
            test: (lookup) => {
                ...
                if (count === null)
                    return unknown('No external-activity telemetry was collected');
                const evidence = lookup.evidence('external.unadapted_import_count');
                return outcome(count > 0, ramp(count, 1, 3), count > 0
                    ? `${count} import(s) followed an external visit without adaptation`
                    : 'No unadapted imports followed external activity', evidence);
            },
        },
        {
            id: 'arrived_in_bulk',
            ...
            test: (lookup) => {
                ...
                if (ratio === null)
                    return unknown('No typing telemetry was collected');
                return outcome(ratio > 0.2, ramp(ratio, 0.2, 0.7), ratio > 0.2
                    ? `${(ratio * 100).toFixed(0)}% of inserted characters arrived in bulk from outside the workspace`
                    : 'Little or no code arrived in bulk from outside the workspace', lookup.evidence('typing.unexplained_insertion_ratio'));
            },
        },
        {
            id: 'never_adapted',
            ...
            test: (lookup) => {
                ...
                return outcome(best < 0.2, ramp(0.2 - best, 0, 0.2), best < 0.2
                    ? 'The inserted code was left essentially unchanged'
                    : `${(best * 100).toFixed(0)}% of inserted code was subsequently adapted`, lookup.evidence('evolution.post_insertion_revision_ratio', 'external.adaptation_ratio'));
            },
        },
        {
            id: 'never_verified',
            ...
            test: (lookup) => {
                ...
                return outcome(!didVerify, 1, didVerify
                    ? 'The developer ran or tested the code before submitting'
                    : 'The code was submitted without any recorded execution or test run', lookup.evidence('runtime.verification_before_submit', 'runtime.test_run_count'));
            },
        },
    ],
};
const ASSISTANCE_DEPENDENCE = {
    id: 'assistance_dependence',
    title: 'Editor suggestions accepted without engagement',
    meaning: 'Substantial completions were accepted faster than they could be read, left unmodified, and never verified — the dependent pattern rather than the assisted one.',
    ...
    conditions: [
        {
            ...
            test: (lookup) => {
                ...
                if (index === null)
                    return unknown('No editor-assistance telemetry was collected');
                return outcome(index > 0.3, ramp(index, 0.3, 0.8), index > 0.3
                    ? `${(index * 100).toFixed(0)}% of accepted suggestions were substantial, instant and unexamined`
                    : 'Suggestions were generally read before being accepted', lookup.evidence('assist.dependency_index'));
            },
        },
        {
            ...
            test: (lookup) => {
                ...
                if (engagement === null)
                    return unknown('No suggestions were accepted');
                return outcome(engagement < 0.25, ramp(0.25 - engagement, 0, 0.25), engagement < 0.25
                    ? `Only ${(engagement * 100).toFixed(0)}% of accepted suggestions were engaged with afterwards`
                    : `${(engagement * 100).toFixed(0)}% of accepted suggestions were modified, deleted or tested`, lookup.evidence('assist.post_acceptance_engagement_rate'));
            },
        },
        {
            ...
            test: (lookup) => {
                ...
                if (assisted === null)
                    return unknown('No assistance telemetry was collected');
                return outcome(assisted > 0.7, ramp(assisted, 0.7, 0.95), assisted > 0.7
                    ? `${(assisted * 100).toFixed(0)}% of characters came from completions`
                    : `${(assisted * 100).toFixed(0)}% of characters came from completions, within normal range`, lookup.evidence('assist.assisted_character_ratio'));
            },
        },
        {
            ...
            test: (lookup) => {
                ...
                if (resolution === null) {
                    return outcome(false, 0, 'No errors were encountered, so debugging capability was not exercised', []);
                }
                return outcome(resolution < 0.5, ramp(0.5 - resolution, 0, 0.5), resolution < 0.5
                    ? `Only ${(resolution * 100).toFixed(0)}% of encountered errors were resolved`
                    : `${(resolution * 100).toFixed(0)}% of encountered errors were resolved`, lookup.evidence('runtime.error_resolution_rate', 'runtime.debugging_engagement'));
            },
        },
    ],
};
const INTEGRITY_COMPROMISE = {
    id: 'environment_integrity_compromise',
    title: 'The measurement environment itself was disturbed',
    meaning: 'The sandbox detected interference with its own instrumentation, alongside a change in the reported device context.',
    ...
    interpretation: 'Concerns the apparatus rather than the work, so it is reported separately from anything about the developer capability. Browser extensions, accessibility tooling, corporate proxies and ordinary browser updates all trigger these detectors, which is why two independent conditions are required and why this can never stand alone. Exonerating evidence: a plausible technical explanation, or the same pattern appearing across many unrelated sessions.',
    conditions: [
        {
            ...
            test: (lookup) => {
                ...
                if (violations === null)
                    return unknown('No environment telemetry was collected');
                return outcome(violations > 0, ramp(violations, 1, 3), violations > 0
                    ? `${violations} runtime integrity violation(s) detected`
                    : 'No instrumentation tampering detected', lookup.evidence('env.runtime_integrity_violations'));
            },
        },
        {
            ...
            test: (lookup) => {
                ...
                if (changes === null)
                    return unknown('No environment telemetry was collected');
                return outcome(changes > 0, ramp(changes, 1, 3), changes > 0
                    ? `Device context changed ${changes} time(s) mid-session`
                    : 'Device context remained stable', lookup.evidence('env.device_context_changes'));
            },
        },
        {
            id: 'evidence_gap',
            ...
            test: (lookup) => {
                ...
                if (coverage === null)
                    return unknown('Session too short to assess coverage');
                return outcome(coverage < 0.7, ramp(0.7 - coverage, 0, 0.4), coverage < 0.7
                    ? `Only ${(coverage * 100).toFixed(0)}% of the session was observed`
                    : `${(coverage * 100).toFixed(0)}% of the session was observed`, lookup.evidence('env.telemetry_coverage'));
            },
        },
    ],
};
const ABSENT_DEVELOPMENT_PROCESS = {
    id: 'absent_development_process',
    title: 'Finished code with no visible development',
    meaning: 'The submitted work shows none of the ordinary traces of being built: no revision, no correction, no iteration, no verification.',
    minimumConditions: 3,
    ...
    conditions: [
        {
            ...
            test: (lookup) => {
                ...
                if (revision === null)
                    return unknown('No code-evolution telemetry was collected');
                return outcome(revision < 0.15, ramp(0.15 - revision, 0, 0.15), revision < 0.15
                    ? 'Almost no changes modified previously written code'
                    : `${(revision * 100).toFixed(0)}% of changes revised existing code`, lookup.evidence('evolution.revision_ratio'));
            },
        },
        {
            id: 'no_correction',
            ...
            test: (lookup) => {
                ...
                if (correction === null)
                    return unknown('No typing telemetry was collected');
                return outcome(correction < 0.02, ramp(0.02 - correction, 0, 0.02), correction < 0.02
                    ? 'Code was written with virtually no corrections'
                    : `${(correction * 100).toFixed(1)}% of inserted characters were corrected`, lookup.evidence('typing.correction_ratio'));
            },
        },
        {
            ...
            test: (lookup) => {
                ...
                if (share === null)
                    return unknown('No code-evolution telemetry was collected');
                return outcome(share > 0.75, ramp(share, 0.75, 1), share > 0.75
                    ? `${(share * 100).toFixed(0)}% of all added lines arrived in one change`
                    : `Largest single change contributed ${(share * 100).toFixed(0)}% of added lines`, lookup.evidence('evolution.largest_increment_share'));
            },
        },
        {
            id: 'no_verification',
            ...
            test: (lookup) => {
                ...
                return outcome(total === 0, 1, total === 0
                    ? 'The code was never executed or tested'
                    : `Code was executed or tested ${total} time(s)`, lookup.evidence('runtime.execution_count', 'runtime.test_run_count'));
            },
        },
    ],
};
export const CLUSTER_CATALOGUE = [
    UNVERIFIED_IMPORT,
...
export function evaluateCluster(definition, lookup) {
    const outcomes = definition.conditions.map((condition) => ({
    ...
    const fired = met.length >= definition.minimumConditions && layersCorroborating.length >= 2;
    ...
    const meanDegree = met.length === 0 ? 0 : met.reduce((sum, entry) => sum + entry.degree, 0) / met.length;
    ...
    const mitigationStrength = mitigations.length === 0
        ? 0
        : Math.min(0.6, mitigations.reduce((sum, note) => sum + note.weight, 0) / 3);
    const confidence = outcomes.length === 0
        ? 0
...
function collectMitigations(clusterId, lookup) {
    const names = MITIGATION_FEATURES[clusterId] ?? [];
    const notes = [];
    for (const name of names) {
        const feature = lookup.get(name);
        if (feature === undefined || feature.value === null || feature.value <= 0)
            continue;
