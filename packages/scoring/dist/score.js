...
import { CLUSTER_CATALOGUE, aggregateSeverity, evaluateCluster } from "./clusters.js";
import { RECOMMENDATION, } from "./contract.js";
import { assessLayer, combineStandings } from "./layers.js";
/**
 * The scoring engine.
 *
...
export function score(extraction, options = {}) {
 ...
 */
function computeTrust(standing, severity, options) {
    if (standing === null)
        return clampUnit(0.5);
    let value = standing;
 ...
 */
function computeRisk(severity, confidence) {
 ...
 */
function computeConfidence(layers, clusters, extraction, layersScored) {
    if (extraction.all.length === 0)
        return clampUnit(0);
    ...
    const featureConfidence = computed.length === 0
        ? 0
        : computed.reduce((sum, feature) => sum + feature.confidence, 0) / computed.length;
    const clusterConfidence = clusters.length === 0
        ? 1
    ...
    return clampUnit((coverage * 0.3 + breadth * 0.25 + featureConfidence * 0.3 + clusterConfidence * 0.15) ** 0.9);
}
function recommend(trust, risk, confidence, fired, standing, evidentialWeight) {
    ...
    if (fired.length > 0 || risk >= 0.2)
        return RECOMMENDATION.CLARIFICATION_SUGGESTED;
    return confidence < 0.5 || trust < 0.6
        ? RECOMMENDATION.SUPPORTED_LOW_CONFIDENCE
        : RECOMMENDATION.SUPPORTED;
}
function explain(layers, clusters, trust, risk, confidence, recommendation) {
    ...
    confidenceFactors.push(`${scoredLayers.length} of ${layers.length} layers produced scoreable evidence.`);
    const thin = layers.filter((layer) => layer.coverage < 0.6);
    if (thin.length > 0) {
        confidenceFactors.push(`Partial telemetry in ${thin.length} layer(s): ${thin.map((layer) => layer.layer).join(', ')}.`);
    }
    const undetermined = clusters.filter((finding) => finding.conditionsIndeterminate > 0);
    if (undetermined.length > 0) {
        confidenceFactors.push(`${undetermined.length} risk pattern(s) could not be fully evaluated for want of evidence.`);
    }
    if (fired.length === 0) {
...
function headline(trust, risk, confidence, fired, recommendation) {
 ...
 */
function suggestQuestions(fired, layers) {
    const questions = [];
    for (const finding of fired) {
        switch (finding.definition.id) {
            case 'unverified_external_import':
                questions.push('Walk through the section of code that was added in bulk: what does it do, and why is it structured that way?', 'What would you change in that section if the requirements shifted?');
                break;
            case 'assistance_dependence':
                questions.push('Explain the implementation the editor suggested and you accepted — why is it correct here?', 'What alternative approach would you have written by hand, and what are its trade-offs?');
                break;
            case 'absent_development_process':
                questions.push('You wrote this with very few revisions — talk through the design you had in mind before you started.', 'Which part of this was hardest to get right, and why?');
                break;
...
function collectLimitations(layers, clusters, extraction, confidence) {
    const limitations = [];
    if (confidence < 0.5) {
        limitations.push('Confidence is below 50%. These scores describe what little was observed and should not carry decisive weight.');
    }
    ...
    for (const finding of clusters) {
        if (finding.conditionsIndeterminate > 0) {
            limitations.push(`"${finding.definition.title}" was evaluated with ${finding.conditionsIndeterminate} of ${finding.conditionsTotal} conditions undetermined.`);
        }
    }
    limitations.push('Trust is never certain. External AI use is not reliably observable, and these scores are a recommendation for human judgement, not a verdict.');
    return limitations;
}
function makeLookup(extraction) {
    const get = (name) => extraction.featuresByName.get(name);
    return {
        get,
