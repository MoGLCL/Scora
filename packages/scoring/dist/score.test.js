...
import { TEST_CONTEXT, buildSession, payloads } from '@scora/trust-features/testing';
import { CLUSTER_CATALOGUE } from "./clusters.js";
import { RECOMMENDATION } from "./contract.js";
import { developerFacingSummary, renderReport } from "./report.js";
import { DEPENDENT, FAST_DEVELOPER, HEAVY_ASSISTANCE_ENGAGED, HONEST_SOLO, HONEST_WITH_DOCS, SPARSE, STRUGGLING_BEGINNER, sessionStart, } from "./scenarios.js";
...
describe('honest archetypes are never flagged', () => {
    ...
    for (const [name, specs] of HONEST) {
        ...
        it(`${name}: is never given an adverse recommendation`, () => {
            ...
            const result = run(specs);
            assert.ok(result.recommendation === RECOMMENDATION.SUPPORTED ||
                result.recommendation === RECOMMENDATION.SUPPORTED_LOW_CONFIDENCE, `${name} received ${result.recommendation}`);
        });
    }
...
describe('cluster catalogue structure', () => {
    it('requires every cluster to span at least two layers', () => {
        for (const definition of CLUSTER_CATALOGUE) {
            assert.ok(definition.layers.length >= 2, `${definition.id} draws from one layer and could fire on a single signal`);
        }
    });
    it('tags every condition with the layer its evidence comes from', () => {
        for (const definition of CLUSTER_CATALOGUE) {
            for (const condition of definition.conditions) {
                ...
                assert.ok(definition.layers.includes(condition.layer), `${definition.id}/${condition.id} reads ${condition.layer}, undeclared on the cluster`);
            }
        }
    });
    it('cannot satisfy its minimum from a single layer', () => {
        ...
        for (const definition of CLUSTER_CATALOGUE) {
            ...
            assert.ok(largest < definition.minimumConditions, `${definition.id} has ${largest} conditions in one layer but needs only ${definition.minimumConditions}`);
        }
    });
    it('requires every cluster to need at least two conditions', () => {
        for (const definition of CLUSTER_CATALOGUE) {
            assert.ok(definition.minimumConditions >= 2, `${definition.id} could fire on one condition`);
            assert.ok(definition.conditions.length >= definition.minimumConditions, `${definition.id} cannot satisfy its own minimum`);
        }
    });
    it('gives every cluster an interpretation naming what would exonerate', () => {
        for (const definition of CLUSTER_CATALOGUE) {
            assert.match(definition.interpretation, /exonerat|discharg|not proof|innocent|identical|explain/i, `${definition.id} must state how it can be resolved in the developer's favour`);
        }
    });
    ...
    it('only ever fires with corroboration from two or more layers', () => {
        for (const specs of [HONEST_SOLO, HONEST_WITH_DOCS, HEAVY_ASSISTANCE_ENGAGED, DEPENDENT, SPARSE]) {
            for (const finding of run(specs).clusters) {
                if (!finding.fired)
                    continue;
                assert.ok(finding.layersCorroborating.length >= 2, `${finding.definition.id} fired on evidence from one layer`);
            }
        }
    });
});
describe('honest solo session', () => {
    ...
    it('recommends support rather than review', () => {
        assert.ok(result.recommendation === RECOMMENDATION.SUPPORTED ||
            result.recommendation === RECOMMENDATION.SUPPORTED_LOW_CONFIDENCE, `got ${result.recommendation}`);
    });
...
describe('honest session with documentation', () => {
    ...
    it('scores at least as well as the same session without documentation', () => {
        ...
        assert.ok(result.trust >= withoutDocs.trust - 2, `consulting documentation cost ${withoutDocs.trust - result.trust} trust points`);
    });
});
describe('heavy but engaged use of editor completions', () => {
    ...
    it('scores comparably to a session with no assistance at all', () => {
        ...
        assert.ok(Math.abs(result.trust - unassisted.trust) <= 25, `assisted ${result.trust} vs unassisted ${unassisted.trust} — using the editor must not be decisive`);
    });
});
describe('dependent session', () => {
    ...
    it('routes to a human rather than deciding', () => {
        assert.ok(result.recommendation === RECOMMENDATION.HUMAN_REVIEW_REQUIRED ||
            result.recommendation === RECOMMENDATION.CLARIFICATION_SUGGESTED, `got ${result.recommendation}`);
    });
    ...
    it('produces interview questions rather than conclusions', () => {
        ...
        for (const question of result.explanation.suggestedQuestions) {
            ...
            assert.match(question, /\?|explain|walk through|talk through|describe|discuss/i, `not phrased as an invitation to explain: ${question}`);
            // ...and none may presuppose an answer.
            assert.doesNotMatch(question, /cheat|copie[dr]|plagiar|did you actually|admit|confess|suspicious/i, `presupposes wrongdoing: ${question}`);
        }
    });
...
describe('no single signal can drive an adverse outcome', () => {
    ...
    it('adding one adverse event to an honest session moves trust only slightly', () => {
        ...
        assert.ok(Math.abs(clean.trust - withPaste.trust) <= 15, `a single paste moved trust by ${Math.abs(clean.trust - withPaste.trust)} points`);
    });
});
describe('confidence behaviour', () => {
    ...
    it('blocks adverse recommendations when confidence is too low', () => {
        ...
        assert.notEqual(result.recommendation, RECOMMENDATION.HUMAN_REVIEW_REQUIRED, 'thin evidence must not produce an adverse recommendation');
    });
...
describe('explainability', () => {
    ...
    it('always states that trust is not certain', () => {
        const result = run(HONEST_SOLO);
        assert.ok(result.limitations.some((limitation) => /never certain/i.test(limitation)), 'every report must disclaim certainty');
    });
...
describe('policy invariants', () => {
    ...
    it('produces scores in range for every archetype', () => {
        for (const specs of [HONEST_SOLO, HONEST_WITH_DOCS, HEAVY_ASSISTANCE_ENGAGED, DEPENDENT, SPARSE]) {
            const result = run(specs);
            for (const key of ['trust', 'risk', 'confidence']) {
                assert.ok(Number.isInteger(result[key]) && result[key] >= 0 && result[key] <= 100, `${key} was ${result[key]}`);
            }
        }
    });
    it('is deterministic', () => {
        ...
        assert.deepEqual([a.trust, a.risk, a.confidence, a.recommendation], [b.trust, b.risk, b.confidence, b.recommendation]);
    });
...
describe('interview influence', () => {
    it('lets a strong explanation lift a session with concerns', () => {
        ...
        assert.ok(withInterview.trust > withoutInterview.trust, 'explaining your work must be able to discharge a concern');
    });
    ...
    it('weights the interview by how many questions were actually asked', () => {
        ...
        assert.ok(manyQuestions.trust < oneQuestion.trust, 'one bad answer must not carry the weight of twelve');
    });
});
