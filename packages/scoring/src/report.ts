import { LAYER_DEFINITIONS, type TrustLayer } from '@scora/trust-core';
import { RECOMMENDATION, type ScoringResult } from './contract.ts';
...
export function renderReport(result: ScoringResult): string {
  ...
  if (result.explanation.riskEvidence.length > 0) {
    ...
    for (const finding of result.explanation.riskEvidence) {
      ...
      lines.push(
        `      severity ${round(finding.severity)}  confidence ${round(finding.confidence)}  ` +
 ...
 */
export function developerFacingSummary(result: ScoringResult): string {
...
function shortName(layer: TrustLayer): string {
