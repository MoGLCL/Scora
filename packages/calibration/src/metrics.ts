import { RECOMMENDATION, type Recommendation } from '@scora/trust-scoring';
import {
  CaseLabel,
  Consequence,
  type CalibrationBin,
 ...
 */
export function consequenceOf(recommendation: Recommendation): Consequence {
 ...
 */
export function isCorrect(outcome: Pick<CaseOutcome, 'case' | 'consequence'>): boolean {
...
export function tally(outcomes: readonly CaseOutcome[]): ConfusionCounts {
  let truePositives = 0;
  ...
  for (const outcome of outcomes) {
    switch (outcome.case.label) {
      case CaseLabel.OWNS_WORK:
        if (outcome.consequence === Consequence.ESCALATED) falsePositives += 1;
        else trueNegatives += 1;
        break;
      case CaseLabel.WARRANTS_REVIEW:
        if (outcome.consequence === Consequence.ESCALATED) truePositives += 1;
        else falseNegatives += 1;
        break;
      case CaseLabel.INDETERMINATE:
        if (outcome.consequence === Consequence.DECLINED) correctRefusals += 1;
        else overclaims += 1;
        break;
      default:
        break;
...
export function rates(counts: ConfusionCounts): ErrorRates {
  const {
    truePositives,
    falsePositives,
    trueNegatives,
  ...
  return {
    ...
    sampleSize:
      truePositives + falsePositives + trueNegatives + falseNegatives + correctRefusals + overclaims,
  };
}
...
export function bySubgroup(
  ...
  traits: readonly CaseTrait[],
): readonly SubgroupReport[] {
  const reports: SubgroupReport[] = [];
 ...
 */
export function calibrationCurve(outcomes: readonly CaseOutcome[]): CalibrationCurve {
  ...
  for (let lower = 0; lower < 100; lower += BIN_WIDTH) {
    ...
    const meanConfidence =
      members.reduce((sum, outcome) => sum + outcome.confidence, 0) / members.length / 100;
  ...
  const weighted = (magnitude: (gap: number) => number): number | null =>
    total === 0
      ? null
 ...
 */
function brier(outcomes: readonly CaseOutcome[]): number | null {
