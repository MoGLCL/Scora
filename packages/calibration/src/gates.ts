import { CaseLabel, Consequence, type CalibrationReport, type GateFailure } from './contract.ts';
 ...
 */
export function evaluate(report: Omit<CalibrationReport, 'failures'>): readonly GateFailure[] {
  ...
  if (outcomes.length < MINIMUM_CASES) {
    failures.push({
      ...
      detail:
        `the corpus holds ${String(outcomes.length)} cases, below the ${String(MINIMUM_CASES)} ` +
  ...
  if (counts.falsePositives > 0) {
    ...
    failures.push({
      ...
      detail:
        `${String(counts.falsePositives)} developer(s) who own their work were escalated to human ` +
  ...
  for (const subgroup of subgroups) {
    if (subgroup.falsePositiveCaseIds.length === 0) continue;
    failures.push({
      gate: GATE.NO_SUBGROUP_HARM,
      detail:
        `every false positive among developers tagged ${subgroup.trait} is a false positive that ` +
  ...
  if (counts.overclaims > 0) {
    ...
    failures.push({
      ...
      detail:
        `${String(counts.overclaims)} session(s) with too little evidence to assess were assessed ` +
  ...
  if (rates.recall !== null && rates.recall < TARGET_RECALL) {
    failures.push({
      ...
      detail:
        `recall is ${format(rates.recall)}, below the ${format(TARGET_RECALL)} target — sessions a ` +
  ...
  if (
    calibration.overconfidenceError !== null &&
    calibration.overconfidenceError > MAXIMUM_CALIBRATION_ERROR
  ) {
    failures.push({
      gate: GATE.CALIBRATION_ERROR,
      detail:
        `stated confidence overstates observed accuracy by ${format(calibration.overconfidenceError)} ` +
  ...
  if (
    calibration.worstOverconfidence !== null &&
    calibration.worstOverconfidence < -MAXIMUM_OVERCONFIDENCE
  ) {
    failures.push({
      gate: GATE.OVERCONFIDENCE,
      detail:
        `one confidence band overstates accuracy by ${format(Math.abs(calibration.worstOverconfidence))}; ` +
  ...
  const unexplained = outcomes.filter(
    (outcome) => outcome.consequence === Consequence.ESCALATED && outcome.firedClusters.length === 0,
  );
  if (unexplained.length > 0) {
    failures.push({
      gate: GATE.EXPLAINED_ESCALATIONS,
      detail:
        'a session was escalated with no fired cluster to point at, which means Risk arrived from ' +
 ...
 */
export function limitationsOf(report: Omit<CalibrationReport, 'failures' | 'limitations'>): readonly string[] {
  ...
  const synthetic = report.outcomes.filter(
    (outcome) => outcome.case.labelSource === 'SYNTHETIC',
  ).length;
  if (synthetic === report.outcomes.length) {
    limitations.push(
      'Every label here is a design decision, not a human judgement. Until reviewed sessions are ' +
  ...
  if (populated.length > 0 && populated.every((bin) => bin.observedAccuracy === 1)) {
    limitations.push(
      'The engine was right about every scored case, so the calibration curve has no incorrect ' +
  ...
  if (
    report.calibration.underconfidenceError !== null &&
    report.calibration.underconfidenceError > MAXIMUM_CALIBRATION_ERROR
  ) {
    limitations.push(
      `Stated confidence understates observed accuracy by ${format(report.calibration.underconfidenceError)} ` +
  ...
  const indeterminate = report.outcomes.filter(
    (outcome) => outcome.case.label === CaseLabel.INDETERMINATE,
  ).length;
  if (indeterminate < 2) {
    limitations.push(
      'Too few indeterminate cases to test whether the engine declines when it should. Refusing to ' +
  ...
  for (const subgroup of report.subgroups) {
    if (subgroup.rates.sampleSize < 3) {
      limitations.push(
        `Subgroup ${subgroup.trait} holds ${String(subgroup.rates.sampleSize)} case(s). Its rates ` +
