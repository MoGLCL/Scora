...
import { CaseLabel, CaseTrait, Consequence, LabelSource, } from "./contract.js";
import { CORPUS } from "./corpus.js";
import { GATE, MAXIMUM_CALIBRATION_ERROR, MINIMUM_CASES, TARGET_RECALL, evaluate, limitationsOf, } from "./gates.js";
import { bySubgroup, calibrationCurve, consequenceOf, isCorrect, rates, tally, } from "./metrics.js";
import { confirmedFalseNegatives, confirmedFalsePositives, fromReviews, overrideRates } from "./overrides.js";
import { calibrate, renderReport, runCase } from "./run.js";
/**
...
function outcome(label, consequence, overrides = {}) {
    const labelled = {
...
describe('what a recommendation costs the developer', () => {
    ...
    it('covers every recommendation the engine can produce', () => {
        ...
        assert.deepEqual(Object.values(RECOMMENDATION).slice().sort(), EXPECTED.map(([recommendation]) => recommendation).sort());
    });
});
describe('correctness is asymmetric on purpose', () => {
    it('treats a clarifying question as correct for someone who owns their work', () => {
        ...
        assert.equal(isCorrect({ case: outcome(CaseLabel.OWNS_WORK, Consequence.QUESTIONED).case, consequence: Consequence.QUESTIONED }), true);
    });
    it('does not accept a question where a human should have read the package', () => {
        assert.equal(isCorrect({ case: outcome(CaseLabel.WARRANTS_REVIEW, Consequence.QUESTIONED).case, consequence: Consequence.QUESTIONED }), false);
    });
...
describe('subgroups do not dilute each other', () => {
    const tagged = (traits, consequence) => {
    ...
    it('reports a case under every trait it carries', () => {
        const reports = bySubgroup([tagged([CaseTrait.ASSISTIVE_INPUT, CaseTrait.FAST_TYPIST], Consequence.ESCALATED)], Object.values(CaseTrait));
        assert.deepEqual(reports.map((report) => report.trait).sort(), [CaseTrait.ASSISTIVE_INPUT, CaseTrait.FAST_TYPIST].sort());
...
describe('does a stated confidence mean what it says', () => {
    ...
    it('buckets by stated confidence and includes 100 in the last bin', () => {
        ...
        assert.deepEqual(curve.bins.map((bin) => bin.count), [2, 1, 0, 0, 1]);
    });
    ...
    it('excludes declined sessions, whose confidence is not a claim about anyone', () => {
        const declined = { ...outcome(CaseLabel.INDETERMINATE, Consequence.DECLINED), confidence: 10 };
...
describe('the gates', () => {
    ...
    it('reports the corpus itself as the finding when it is too small', () => {
        const failure = evaluate(reportOf(padding(3))).find((candidate) => candidate.gate === GATE.MINIMUM_CORPUS);
        assert.ok(failure);
...
describe('what the report does not establish', () => {
    it('always says the rates describe the corpus and not the world', () => {
        ...
        assert.ok(limitations.some((line) => /should be quoted to a customer or to a developer/.test(line)), 'the sentence that stops an accuracy figure being repeated');
    });
    ...
    it('warns that a thin subgroup is untested rather than safe', () => {
        ...
        assert.ok(limitations.some((line) => /treat a clean result there as untested, not as safe/.test(line)));
    });
});
describe('ground truth from Layer 10', () => {
    /** Real Layer 10 events, built through the validating session builder. */
    function log(reviews) {
    ...
    it('identifies a confirmed false positive: the engine flagged, a human cleared', () => {
        ...
        assert.deepEqual(confirmedFalsePositives(cases).map((entry) => entry.caseId), ['review:rev_cleared']);
        assert.deepEqual(confirmedFalseNegatives(cases).map((entry) => entry.caseId), ['review:rev_rejected']);
    });
...
describe('the engine against the corpus', () => {
    ...
    it('escalates nobody who owns their work', () => {
        ...
        assert.deepEqual(accused.map((entry) => entry.case.caseId), [], 'each of these is a specific person the engine would have accused');
    });
    it('passes every gate', () => {
        assert.deepEqual(report.failures.map((failure) => `${failure.gate}: ${failure.detail}`), []);
    });
    it('never escalates without naming the clusters that caused it', () => {
        for (const entry of report.outcomes) {
            if (entry.consequence !== Consequence.ESCALATED)
                continue;
            assert.ok(entry.firedClusters.length > 0, `${entry.case.caseId} escalated with no cluster to point at`);
        }
    });
    ...
    it('renders a report that leads with what went wrong', () => {
        ...
        assert.ok(text.indexOf('Counts') < text.indexOf('Cases'), 'a reader at the end of a red build should not scroll past a curve');
        assert.match(text, /What this does not establish/);
        for (const entry of report.outcomes)
            assert.ok(text.includes(entry.case.caseId));
    });
