...
import { TEST_CONTEXT, buildSession } from '@scora/trust-features/testing';
import { POLICY_VERSION, score } from '@scora/trust-scoring';
import { CaseLabel, CaseTrait, Consequence, } from "./contract.js";
import { CORPUS } from "./corpus.js";
import { evaluate, limitationsOf } from "./gates.js";
import { bySubgroup, calibrationCurve, consequenceOf, rates, tally } from "./metrics.js";
 ...
 */
export function calibrate(cases = CORPUS) {
...
export function renderReport(report) {
    ...
    if (report.failures.length === 0) {
        ...
        for (const failure of report.failures) {
            ...
            if (failure.caseIds.length > 0)
                lines.push(`    cases: ${failure.caseIds.join(', ')}`);
            lines.push('');
        }
    }
    const { counts, rates: measured } = report;
    lines.push('Counts');
    lines.push(`  escalated and should have been      ${String(counts.truePositives)}`, `  escalated and should not have been  ${String(counts.falsePositives)}   <- the one that hurts`, `  left alone and should have been     ${String(counts.trueNegatives)}`, `  let through and should not have     ${String(counts.falseNegatives)}`, `  declined and should have            ${String(counts.correctRefusals)}`, `  asserted without evidence           ${String(counts.overclaims)}`, '');
    lines.push('Rates');
    lines.push(`  false positive rate  ${show(measured.falsePositiveRate)}`, `  false negative rate  ${show(measured.falseNegativeRate)}`, `  precision            ${show(measured.precision)}`, `  recall               ${show(measured.recall)}`, `  overclaim rate       ${show(measured.overclaimRate)}`, `  cases                ${String(measured.sampleSize)}`, '');
    lines.push('Calibration (stated confidence vs. observed accuracy)');
    for (const bin of report.calibration.bins) {
        if (bin.count === 0)
            continue;
        lines.push(`  ${String(bin.lowerBound).padStart(3)}–${String(bin.upperBound).padEnd(3)} ` +
            `n=${String(bin.count).padEnd(3)} stated ${show(bin.meanConfidence)} ` +
            `actual ${show(bin.observedAccuracy)} gap ${show(bin.gap)}`);
    }
    lines.push(`  expected calibration error ${show(report.calibration.expectedCalibrationError)}   (reported, not gated)`, `  overconfidence             ${show(report.calibration.overconfidenceError)}   (gated)`, `  underconfidence            ${show(report.calibration.underconfidenceError)}   (reported, not gated)`, `  worst overconfidence       ${show(report.calibration.worstOverconfidence)}`, `  brier score                ${show(report.calibration.brierScore)}`, '');
    if (report.subgroups.length > 0) {
        lines.push('By trait');
        for (const subgroup of report.subgroups) {
            lines.push(`  ${subgroup.trait.padEnd(22)} n=${String(subgroup.rates.sampleSize).padEnd(3)} ` +
    ...
    for (const outcome of report.outcomes) {
        ...
        lines.push(`  ${mark} ${outcome.case.caseId.padEnd(24)} ${outcome.case.label.padEnd(16)} ` +
