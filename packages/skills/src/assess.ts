import { clampUnit, type EpochMs, type TrustEvent, type Unit } from '@scora/trust-core';
import {
  SkillVerdict,
...
import { collectClaims, observeSkills } from './observe.ts';
import { reconcileClaim, type ReconcileOptions } from './reconcile.ts';
...
function coverageOf(findings: readonly SkillFinding[]): SkillCoverage {
  const notExercised = findings.filter(
    (finding) => finding.verdict === SkillVerdict.NOT_EXERCISED,
  ).length;
...
function limitationsOf(
  ...
  if (coverage.exercised === 0) {
    limitations.push(
      `None of the ${String(coverage.claimed)} claimed skills were exercised by this assessment. ` +
        'This layer has verified nothing, in either direction.',
    );
  } else if (coverage.ratio < 0.5) {
    limitations.push(
      `Only ${String(coverage.exercised)} of ${String(coverage.claimed)} claimed skills were exercised. ` +
        'The unexercised claims are unverified, not disproved.',
    );
  }

  const indeterminate = findings.filter(
    (finding) => finding.verdict === SkillVerdict.INDETERMINATE,
  ).length;
  if (indeterminate > 0) {
    limitations.push(
      `${String(indeterminate)} claim(s) were exercised but produced telemetry too thin to read. ` +
        'Treat these as open questions.',
    );
  }

  const challenged = findings.filter((finding) =>
    finding.observations.some((observation) => observation.layer === 'L08_SKILL_UNDERSTANDING'),
  ).length;
  if (challenged === 0 && coverage.exercised > 0) {
    limitations.push(
      'No verification challenges were issued. Every finding here rests on behaviour observed ' +
 ...
 */
export function skillStanding(assessment: SkillAssessment): Unit | null {
  const exercised = assessment.findings.filter(
    (finding) => finding.verdict !== SkillVerdict.NOT_EXERCISED,
  );
...
function standingOf(finding: SkillFinding): number {
 ...
 */
export function skillConfidence(assessment: SkillAssessment): Unit {
  const exercised = assessment.findings.filter(
    (finding) => finding.verdict !== SkillVerdict.NOT_EXERCISED,
  );
  if (exercised.length === 0) return clampUnit(0);

  const mean =
    exercised.reduce((sum, finding) => sum + finding.confidence, 0) / exercised.length;
 ...
 */
export function suggestedQuestions(assessment: SkillAssessment): readonly string[] {
