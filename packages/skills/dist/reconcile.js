...
import { SKILL_LEVEL_ORDER, SkillLevel, SkillObservationKind, SkillVerdict, levelOrdinal, } from "./contract.js";
/**
...
export function reconcileClaim(claim, observations, options = {}) {
    ...
    const counterLayers = distinctLayers(observations.filter((observation) => isCounter(observation.kind)));
    ...
    const contradicted = levelGap > toleratedGap &&
    ...
    if (contradicted) {
        return {
            ...
            observations,
            evidence,
            summary: `${claim.skillName} was claimed at ${claim.claimedLevel} and the session evidences ${evidencedLevel}, ` +
    ...
    if (levelGap > toleratedGap) {
        return {
            ...
            observations,
            evidence,
            summary: `${claim.skillName} is evidenced at ${evidencedLevel} against a claim of ${claim.claimedLevel}. ` +
 ...
 */
export function inferLevel(observations) {
    ...
    const counter = weightOf(observations, SkillObservationKind.COUNTER) +
 ...
 */
export function aggregateConfidence(observations) {
    if (observations.length === 0)
        return clampUnit(0);
 ...
 * confuse the two.
 */
function hasDirectFailure(observations) {
    return observations.some((observation) => observation.kind === SkillObservationKind.CHALLENGE_FAILED);
}
function weightOf(observations, kind) {
    return observations
        .filter((observation) => observation.kind === kind)
        .reduce((sum, observation) => sum + observation.weight * observation.confidence, 0);
}
function distinctLayers(observations) {
