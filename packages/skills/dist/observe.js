import { CodeEvolutionEventType, RuntimeEventType, SkillEventType, TrustLayer, clampUnit, } from '@scora/trust-core';
import { SkillLevel, SkillObservationKind, } from "./contract.js";
/**
...
export function collectClaims(events) {
    const claims = new Map();
    for (const event of events) {
        if (event.type !== SkillEventType.SKILL_CLAIMED)
            continue;
 ...
 */
export function observeSkills(events, claims) {
    const observations = new Map();
    const known = new Set(claims.map((claim) => claim.skillId));
    const add = (skillId, observation) => {
        if (!known.has(skillId))
            return;
        const existing = observations.get(skillId);
        if (existing === undefined)
            observations.set(skillId, [observation]);
        else
            existing.push(observation);
    };
    for (const window of taskWindows(events)) {
        const inside = events.filter((event) => event.chainPosition > window.startedAt && event.chainPosition <= (window.endedAt ?? Infinity));
    ...
    for (const event of events) {
        if (event.type !== SkillEventType.VERIFICATION_CHALLENGE_RESULT)
            continue;
        const payload = event.payload;
        ...
        add(payload.skillId, {
            ...
            detail: passed
                ? `Passed a targeted verification challenge, scoring ${score.toFixed(2)}${payload.usedAssistance === true ? ' with editor assistance available' : ''}.`
 ...
 * counted twice.
 */
export function taskWindows(events) {
    const open = new Map();
    const windows = [];
    for (const event of events) {
        if (event.type === SkillEventType.TASK_STARTED) {
            ...
            open.set(payload.taskId, {
                targetSkills: Array.isArray(payload.targetSkills)
                    ? payload.targetSkills.filter((skill) => typeof skill === 'string')
                    : [],
 ...
 */
function observeTaskWork(events, difficulty) {
    const observations = [];
    const idsOf = (...types) => events.filter((event) => types.includes(event.type)).map((event) => event.eventId);
