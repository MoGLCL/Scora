...
import { CodeEvolutionEventType, RuntimeEventType, SkillEventType, TrustLayer, unsafeEpochMs, } from '@scora/trust-core';
import { buildSession, T0, HASH, hexHash, payloads } from '@scora/trust-features/testing';
import { SkillLevel, SkillObservationKind, SkillVerdict } from "./contract.js";
import { collectClaims, observeSkills, taskWindows } from "./observe.js";
import { assessSkills, skillConfidence, skillStanding, suggestedQuestions } from "./assess.js";
/**
...
function taskStarted(taskId, targetSkills, at) {
    return {
...
function taskSubmitted(taskId, at) {
    return {
...
function challengeResult(skillId, passed, score, at) {
    return {
...
function assess(specs) {
    return assessSkills(buildSession(specs), OPTIONS);
}
function findingFor(events, skillId) {
...
describe('a session that evidences the claim', () => {
    it('corroborates from a passed targeted challenge', () => {
        const finding = findingFor(buildSession([
            claimed(TS, 'TypeScript', 'intermediate', 0),
            taskStarted('task_a', [TS], 1_000),
            testsPassed(120_000),
            errorResolved('boom', 150_000),
