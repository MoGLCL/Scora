...
describe('router construction', () => {
  ...
  it('refuses an INGEST route paired with a role no sandbox may hold', () => {
    ...
    assert.throws(
      () => buildRouter([{ ...stub, roles: [Role.INGEST, Role.READ_EVIDENCE] }]),
      /pairs it with a role no sandbox may hold/,
    );
  });

  it('refuses a pattern the deployment has forbidden', () => {
    assert.throws(
      () => buildRouter([stub], { forbidden: ['/v1/thing'] }),
...
describe('resolveRoles', () => {
  const allow = (principal: Principal, required: readonly Role[]): boolean =>
    resolveRoles(principal, required).includes('__ALLOW__');
  ...
  it('denies a sandbox every read, even holding the role', () => {
    ...
    const overprivileged: Principal = {
      ...sandboxPrincipal(),
...
describe('tenant isolation', () => {
  ...
  async function seed(app: ReturnType<typeof harness>): Promise<void> {
    const response = await app.request({
      method: 'POST',
...
describe('developer self-access', () => {
  ...
  async function seeded(): Promise<ReturnType<typeof harness>> {
    const app = harness(tokens);
    await app.request({
      method: 'POST',
      path: `/v1/sessions/${SESSION}/events`,
  ...
  it('never shows a developer a risk number or a cluster name', async () => {
    ...
    assert.equal(Object.hasOwn(response.body as object, 'risk'), false);
    assert.equal(Object.hasOwn(response.body as object, 'clusters'), false);
    assert.equal(Object.hasOwn(response.body as object, 'trust'), false);
...
describe('access log', () => {
  ...
  it('records a handler crash without leaking its message', async () => {
    ...
    // someone's evidence.
    const exploding: RouteDefinition = {
      method: 'GET',
      pattern: '/v1/explode',
      roles: [Role.READ_REPORT],
