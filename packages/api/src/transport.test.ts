...
import { createApi, inMemoryAccessLog } from './app.ts';
import { PrincipalKind, Role, type Authenticator } from './contract.ts';
import { createTrustClient, TrustApiError, type TrustClient } from './client.ts';
import { inMemoryPolicyStore, type TenantPolicy } from './policy.ts';
import {
  MAX_BODY_BYTES,
  bearerAuthenticator,
  hashToken,
 ...
 * The transport edge.
 *
 * Everything above this file is plain data, so these tests are the only ones
 * that involve a socket. They exist for the properties that only appear once a
 * real request is parsed: a body arrives before anyone is authenticated, a
 * credential is a string on the wire rather than a `Principal`, and a response
 * carries headers that decide whether a proxy is allowed to keep a copy of
 * someone's evidence.
 */

interface Deployment {
  readonly sandbox: TrustClient;
  readonly reviewer: TrustClient;
  readonly admin: TrustClient;
  readonly url: string;
  readonly tokens: Readonly<Record<'sandbox' | 'reviewer' | 'admin', string>>;
}

/** A real HTTP server over the same composition production uses. */
async function deploy(context: TestContext): Promise<Deployment> {
  ...
  const url = await listen(
    bearerAuthenticator([sandbox.stored, reviewer.stored, admin.stored], { now: () => T0 }),
    context,
  );
...
async function listen(authenticator: Authenticator, context: TestContext): Promise<string> {
...
describe('node transport', () => {
  it('carries a session from ingest to report over a socket', async (t) => {
    ...
    const report = (await app.reviewer.report(SESSION)) as Record<string, unknown>;
...
describe('client', () => {
  it('throws a TrustApiError carrying the problem detail on a refusal', async (t) => {
    const app = await deploy(t);
    await assert.rejects(app.reviewer.savePolicy({}), (error: unknown) => {
  ...
  it('reports a rejected policy write with its field errors', async (t) => {
    const app = await deploy(t);
    const proposed = (await app.admin.policy()) as TenantPolicy;
    const weakened = {
      ...proposed,
      retention: { ...proposed.retention, auditDays: 30 },
    };

    await assert.rejects(app.admin.savePolicy(weakened), (error: unknown) => {
      assert.ok(error instanceof TrustApiError);
