...
export async function toApiRequest(incoming: IncomingMessage): Promise<ApiRequest> {
...
export function writeResponse(outgoing: ServerResponse, response: ApiResponse): void {
  ...
  return {
    async authenticate(token) {
      ...
      for (const [hash, credential] of byHash) {
        if (constantTimeEquals(hash, presented)) matched = credential;
      }
      if (matched === null) return null;

      if (matched.expiresAt !== undefined && matched.expiresAt <= now()) return null;
...
export function issueCredential(
  credential: Omit<StoredCredential, 'tokenHash'>,
): { readonly token: string; readonly stored: StoredCredential } {
 ...
 * else's assessment.
 */
export function issueSandboxCredential(
  tenantId: TenantId,
  sessionId: SessionId,
  expiresAt: number,
): { readonly token: string; readonly stored: StoredCredential } {
