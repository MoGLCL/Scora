...
import type { MatchedRequest } from './router.ts';
...
export function buildRoutes(dependencies: RouteDependencies): readonly RouteDefinition[] {
  ...
  return [
    {
      ...
      async handler(request, principal) {
        ...
        const events = (body['events'] as unknown[]).map((event) =>
          isRecord(event) ? { ...event, tenantId: principal.tenantId, sessionId } : event,
        );
        ...
        const result = await ingestion.ingest(
          events,
          offset === undefined ? {} : { clockOffsetMs: offset },
        );
...

    {
      ...
      async handler(request, principal) {
        ...
        if (events.length === 0) return notFoundSession();
...

    {
      ...
      async handler(request, principal) {
        ...
        if (page.events.length === 0 && cursor === null) return notFoundSession();
      ...
      async handler(request, principal) {
        ...
        if (!policy.review.developerVisibleOutcome) {
          return problem(
            403,
            'Forbidden',
            'this tenant has not enabled developer-visible outcomes',
          );
        }
...

    {
      ...
      async handler(request, principal) {
        const matched = request as MatchedRequest;
        if (!isRecord(matched.body)) return problem(400, 'Bad Request', 'body must be an object');

        const current = await loadPolicy(principal.tenantId);
        const proposed: TenantPolicy = {
          ...current,
          ...(matched.body as Partial<TenantPolicy>),
  ...
  async function loadPolicy(tenantId: TenantId): Promise<TenantPolicy> {
 ...
 */
function redactPolicy(policy: TenantPolicy): unknown {
  return {
 ...
 */
function notFoundSession(): ApiResponse {
