...
import { SESSION, TENANT, harness, heartbeat, reviewerPrincipal, sandboxPrincipal } from './testing.ts';
...
async function seeded(events: readonly unknown[] = [heartbeat(1), heartbeat(2), heartbeat(3)]) {
...
describe('ingestion endpoint', () => {
  ...
  it('reports what it rejected instead of failing the batch', async () => {
    ...
    const body = response.body as { accepted: number; rejected: readonly unknown[] };
  ...
  it('is idempotent on a replayed batch', async () => {
    ...
    const body = replay.body as { accepted: number; duplicates: number };
...
describe('report endpoint', () => {
  ...
  it('carries the evidence integrity check inline', async () => {
    ...
    const body = response.body as { evidenceIntegrity: { intact: boolean; eventsChecked: number } };
  ...
  it('states its limitations', async () => {
    ...
    const body = response.body as { limitations: readonly string[]; confidence: number };
  ...
  it('reports only clusters that actually fired', async () => {
    ...
    const body = response.body as { clusters: readonly unknown[] };
  ...
  it('renders a text report for a human', async () => {
    ...
    assert.match(response.body as string, /SCORA TRUST REPORT/);
  });
...
describe('evidence endpoints', () => {
  it('pages the raw log by chain position', async () => {
    ...
    const page = first.body as { events: readonly { chainPosition: number }[]; nextCursor: string | null };
    ...
    const rest = second.body as { events: readonly { chainPosition: number }[] };
  ...
  it('confirms an untampered chain', async () => {
    ...
    const body = response.body as { intact: boolean; eventsChecked: number; headHash: string | null };
...
describe('developer session listing', () => {
  it('lists sessions for a developer', async () => {
    ...
    assert.equal(response.status, 200);
    const body = response.body as { sessions: readonly { sessionId: string }[] };
  ...
  it('returns an empty list, not a 404, for a developer with no sessions', async () => {
    ...
    assert.equal(response.status, 200);
    assert.deepEqual((response.body as { sessions: readonly unknown[] }).sessions, []);
  });
});
