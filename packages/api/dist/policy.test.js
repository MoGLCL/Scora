...
import { AssistanceProviderKind, MAXIMUM_COMPLETION_CHARS, MAXIMUM_REVIEW_THRESHOLD, MINIMUM_AUDIT_RETENTION_DAYS, assertPolicyInvariants, defaultPolicy, permittedScopes, } from "./policy.js";
import { T0, TENANT, adminPrincipal, harness, reviewerPrincipal } from "./testing.js";
/**
 * Tenant policy.
 *
...
const base = () => defaultPolicy(TENANT, AT, 'admin');
...
describe('policy invariants', () => {
    it('refuses a review threshold that removes the human', () => {
        const violations = assertPolicyInvariants({
            ...base(),
            review: { ...base().review, humanReviewRiskThreshold: 1 },
        });
    ...
    it('accepts a threshold exactly at the maximum', () => {
        ...
        const violations = assertPolicyInvariants({
            ...
            review: {
                ...base().review,
                humanReviewRiskThreshold: MAXIMUM_REVIEW_THRESHOLD,
            },
        });
    ...
    it('refuses an adverse finding on thinner evidence than the floor', () => {
        const violations = assertPolicyInvariants({
            ...base(),
            review: { ...base().review, minimumConfidenceForAdverse: 0.1 },
        });
    ...
    it('refuses an audit trail shorter than a year', () => {
        ...
        assert.deepEqual(violations.map((violation) => violation.path), ['retention.auditDays', 'retention.auditDays']);
    ...
    it('reports every violation at once', () => {
        ...
        const violations = assertPolicyInvariants({
            ...
            review: {
                ...base().review,
                humanReviewRiskThreshold: 0.99,
                minimumConfidenceForAdverse: 0,
            },
        });
